...
with col2:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=tf_compute,
            mode="lines",
            name="Transformer Compute (Attention)",
    ...
    if max_seq_len > 10000:
        st.error(
            f"⚠️ 警告：当序列长度达到 {max_seq_len} 时，Transformer 的 Attention 矩阵需要计算 {max_seq_len**2:,} 次点积！"
        )
...
with col3:
    ...
    fig_attn = go.Figure(
        data=go.Heatmap(z=attn_weights, colorscale="Viridis", showscale=False)
    )
...
with col4:
    ...
    fig_ssm = go.Figure(
        data=go.Heatmap(z=hidden_states, colorscale="Magma", showscale=False)
    )
...
st.info(
    "💡 对比观察：左边的矩阵大小随序列长度平方增长 (N*N)，而右边的隐状态高度永远固定 (d_state)，只随时间向右延伸。"
)
