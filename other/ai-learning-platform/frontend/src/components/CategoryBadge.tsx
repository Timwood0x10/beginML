...
export default function CategoryBadge({
  ...
  const styles = CATEGORY_BADGE_STYLES[category.id] ?? CATEGORY_BADGE_STYLES.general
  const padding = size === 'md' ? 'px-3.5 py-1.5 text-label-md' : 'px-2.5 py-1 text-caption'
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full font-semibold ${padding} ${styles}`}
    >
      <span className="material-symbols-outlined" style={{ fontSize: size === 'md' ? 16 : 13 }}>
        {category.icon}
      </span>
      {category.en}
    </span>
