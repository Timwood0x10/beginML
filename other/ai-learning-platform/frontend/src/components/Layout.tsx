...
function getToggleTideGradient(
  palette: PaletteId,
...
function ThemeDropdown({
  ...
  const current =
    THEME_OPTIONS.find((o) => o.id === palette) ?? THEME_OPTIONS[0];
          ...
          {THEME_OPTIONS.map((o) => (
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span
                  className={`block text-[10px] truncate ${palette === o.id ? "opacity-80" : "text-outline"}`}
                >
                  {o.desc}
                </span>
              </span>
...
export default function Layout() {
  ...
  const navItems = [
    ...
    { to: "/paper", label: t.nav.paper, icon: "memory" },
  ];
