  ...
  return (
    <div className="flex flex-col gap-8 pt-2">
      ...
      {loading ? (
        <Spinner
          label={
            debounced ? `${t.common.loading} “${debounced}”` : t.common.loading
          }
        />
      ) : error ? (
        ...
        <EmptyState
          ...
          subtitle={
            debounced
              ? `“${debounced}” — ${t.browse.noResultsSub}`
              : t.browse.tryOther
          }
        />
      ) : (
        <>
          <div className="flex items-center justify-between text-caption text-on-surface-variant dark:text-outline">
            <span>
              {t.browse.showing}{" "}
              <strong className="text-on-surface dark:text-dark-on-surface">
                {sortedNotes.length}
              </strong>{" "}
              {t.browse.of} {data?.total}
...
function FilterChip({
  ...
  return (
    ...
    >
      ...
      {typeof count === "number" && (
        <span
          className={`px-1.5 py-0.5 rounded-full text-caption ${active ? "bg-white/20" : "bg-surface-variant dark:bg-white/10"}`}
        >
          {count}
        </span>
      )}
