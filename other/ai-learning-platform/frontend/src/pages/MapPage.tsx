...
export default function MapPage() {
  ...
  const points = useMemo(() => {
    if (!data) return [];
    return activeCat
      ? data.points.filter((p) => p.category === activeCat)
      : data.points;
  }, [data, activeCat]);
  ...
  if (error || !data)
    return <ErrorState message={error ?? t.common.loading} onRetry={load} />;
  ...
  return (
    <div className="flex flex-col gap-6 pt-2">
      ...
      {/* Map canvas */}
      <div
        className="relative bg-surface-container-lowest dark:bg-dark-surface rounded-3xl border border-outline-variant/50 dark:border-white/10 shadow-ambient dark:shadow-dark-ambient overflow-hidden"
        ...
        >
          <defs>
            ...
            >
              ...
              />
              <path
                d="M0 110 C 60 100, 100 120, 160 108"
                fill="none"
                stroke="#C9B79F"
                strokeWidth="0.3"
                strokeOpacity="0.2"
              ...
              />
              <path
                d="M110 0 C 100 60, 120 100, 108 160"
                fill="none"
                stroke="#C9B79F"
                strokeWidth="0.3"
                strokeOpacity="0.18"
          ...
          <rect
            ...
            width="100%"
            height="100%"
            fill="url(#parchment-vignette-dark)"
            className="hidden dark:block"
            pointerEvents="none"
          />
          ...
          <g>
            {points.map((p, i) => {
              ...
              return points.slice(i + 1).map((q) => {
                ...
                const touchesHover =
                  hover && (hover.id === p.id || hover.id === q.id);
                ...
                const baseOpacity = Math.max(
                  floor,
                  (dark ? 0.95 : 0.46) - dist * (dark ? 0.5 : 0.4),
                );
                const opacity = touchesHover
                  ? dark
                    ? 1.0
                    : 0.98
                  : hover
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <span
                className="w-2 h-2"
                style={{
                  background: CATEGORY_COLORS[hover.category] ?? "#3D322C",
                  borderRadius: "1px",
                }}
              />
            ...
            <div className="font-codex text-base font-semibold leading-snug">
              {hover.title}
            </div>
...
function LegendPill({
  ...
  return (
    ...
    >
      ...
      {label}
      <span
        className={`px-1.5 py-0.5 text-caption ${active ? "bg-white/30" : "bg-[#D8C8B0] dark:bg-white/10"}`}
