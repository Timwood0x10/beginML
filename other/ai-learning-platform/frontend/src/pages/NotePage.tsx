...
export default function NotePage() {
  ...
  const { measureRef, pages } = useBookPagination(
    note?.html ?? "",
    pageHeight,
    pageWidth,
  );
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < pageEls.length; i++) {
      const p = flip.getPage(i) as
        { setDensity?: (d: string) => void } | undefined;
      p?.setDensity?.("soft");
    }
  ...
  if (error || !note)
    return <ErrorState message={error ?? "Note not found."} onRetry={load} />;

  const toc: Heading[] = note.headings.filter(
  ...
  return (
    <div className="flex flex-col gap-6 pt-2">
      ...
      <div className="flex items-center gap-2 text-caption text-on-surface-variant dark:text-outline">
        ...
        <Link
          to="/browse"
          className="hover:text-primary dark:hover:text-inverse-primary transition-colors"
        >
          {t.nav.library}
        </Link>
        <span className="text-outline-variant dark:text-white/20">/</span>
        <span className="text-on-surface dark:text-dark-on-surface truncate max-w-[50vw]">
          {note.title}
        </span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        >
          ...
          <div
            className="book-3d mt-2"
            style={{ width: pageWidth, height: pageHeight }}
          >
            <div
              ref={bookElRef}
              className="book-flip-host"
              style={{ width: pageWidth, height: pageHeight }}
            />
            ...
            {toc.length > 0 && (
              <div className="max-h-[38vh] overflow-y-auto bg-surface-container-low dark:bg-dark-surface rounded-2xl p-5 border border-outline-variant/40 dark:border-white/10">
                <div className="flex items-center gap-2 mb-3 text-on-surface dark:text-dark-on-surface">
                  ...
                  <h4 className="font-label-md text-label-md uppercase tracking-wider">
                    {t.note.contents}
                  </h4>
                </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-caption text-outline">
                          {t.home.categoryNames[r.category.id] ?? r.category.en}
                        </span>
