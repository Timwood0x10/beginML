...
const UI: Record<"zh" | "en", Record<string, string>> = {
  ...
  en: {
    ...
    subtitle: "Predict → Experiment → Discover",
    empty:
      "No discoveries yet. Run an experiment, submit a prediction, and save it here.",
    emptyHint:
      "Submit a prediction in any experiment to generate an Experiment Card.",
...
export function useDiscoveries() {
  ...
  const addEntry = useCallback(
    (e: Omit<DiscoveryEntry, "id" | "createdAt">) => {
      setEntries((prev) =>
        [
          {
            ...e,
  ...
  const updateInsight = useCallback((id: string, insight: string) => {
    setEntries((prev) =>
      prev.map((e) => (e.id === id ? { ...e, insight } : e)),
    );
  }, []);
...
const L3_UI: Record<
  "zh" | "en",
  { title: string; placeholder: string; save: string; saved: string }
> = {
  zh: {
    title: "用自己的话解释",
  ...
  en: {
    ...
    placeholder:
      "What did this experiment reveal? Write your insight (no auto-grading — it is yours).",
...
export function JournalPanel({
  ...
  const card = (e: DiscoveryEntry) => (
    <div
      key={e.id}
      className="rounded-2xl px-4 py-3 border border-outline-variant/40 dark:border-white/10 bg-surface-container dark:bg-white/5"
    >
      <div className="flex items-center gap-2 mb-1">
        <span
          className={`text-sm ${e.correct ? "text-[#2f6b3e] dark:text-[#9ed0a8]" : "text-[#C8604A] dark:text-[#f0b3a4]"}`}
        >
          {e.correct ? ui.correct : ui.wrong}
        </span>
        <span className="font-label-md font-semibold text-on-surface dark:text-dark-on-surface truncate">
          {e.experimentTitle}
        </span>
      ...
      <div className="text-caption text-on-surface-variant dark:text-outline mb-1">
        {e.question}
      </div>
      <div className="flex flex-wrap items-center gap-2 text-caption">
        <span className="inline-flex items-center gap-1 text-outline">
          {ui.prediction}:{" "}
          <span className="font-mono text-on-surface dark:text-dark-on-surface">
            {e.prediction}
          </span>
        </span>
      ...
      <div className="mt-2">
        {editingId === e.id ? (
          ...
          <div className="rounded-xl border-l-2 border-primary/50 dark:border-inverse-primary/50 bg-surface-container dark:bg-white/5 px-3 py-2 text-caption text-on-surface dark:text-dark-on-surface">
            <span className="text-outline mr-1.5">💡</span>
            {e.insight}
          </div>
        ) : null}
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-5 border border-outline-variant/40 dark:border-white/10">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-caption uppercase tracking-wider font-semibold text-outline mb-0.5">
                {ui.nextQuestion}
              </div>
              <div className="text-body-md text-on-surface dark:text-inverse-on-surface">
                {nextQuestion}
              </div>
            </div>
            {onExplore && (
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        {entries.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-caption text-outline font-mono">
              {fmt(ui.groupCount, { n: entries.length })}
            </span>
          </div>
        )}
        ...
        {entries.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-outline-variant/50 dark:border-white/15 p-6 text-center">
            ...
            <div className="text-body-md text-on-surface dark:text-dark-on-surface">
              {ui.empty}
            </div>
            ...
            {groups.map(([title, list]) => (
              <div key={title}>
                <h4 className="text-caption uppercase tracking-wider font-semibold text-outline mb-2">
                  {title}{" "}
                  <span className="font-mono text-outline/70">
                    ({list.length})
                  </span>
                </h4>
            ...
            {timeline.map((e, i) => (
              <div key={e.id} className="flex items-stretch gap-3">
                ...
                <div className="flex flex-col items-center shrink-0">
                  ...
                  {i < timeline.length - 1 && (
                    <span className="w-px flex-1 bg-outline-variant/40 dark:bg-white/10" />
                  )}
                </div>
