...
const UI: Record<'zh' | 'en', Record<string, string>> = {
  ...
  en: {
    ...
    swap: '↔ Swap',
    clear: 'Clear',
    empty: 'No snapshots yet. Tune one set of parameters and hit "Lock A", tune another and hit "Lock B".',
    delta: 'Delta',
  },
}

function MetricRow({ m, delta }: { m: ABMetric; delta: number | null }) {
  const dir = delta === null ? null : delta > 0.0005 ? 'up' : delta < -0.0005 ? 'down' : 'flat'
  const good = dir === null ? null : (m.higherBetter ? dir === 'up' : dir === 'down')
  return (
    <div className="flex items-center gap-2 text-caption">
      <span className="w-28 shrink-0 truncate text-outline">{m.label}</span>
      <span className="w-14 text-right font-mono text-on-surface dark:text-dark-on-surface tabular-nums">{m.value.toFixed(3)}</span>
      {delta !== null && (
        <span className={`font-mono tabular-nums ${good === null ? 'text-outline' : good ? 'text-[#2f6b3e] dark:text-[#9ed0a8]' : 'text-[#C8604A] dark:text-[#f0b3a4]'}`}>
          {dir === 'flat' ? '·' : `${dir === 'up' ? '▲' : '▼'}${Math.abs(delta).toFixed(3)}`}
        </span>
      )}
    </div>
  )
      ...
      {!a && !b ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl border border-outline-variant/40 dark:border-white/10 bg-surface-container dark:bg-white/5 p-3">
            <div className="font-label-md font-semibold text-primary dark:text-inverse-primary mb-2 truncate">{a?.name ?? 'A'}</div>
          ...
          <div className="rounded-2xl border border-outline-variant/40 dark:border-white/10 bg-surface-container dark:bg-white/5 p-3">
            <div className="font-label-md font-semibold text-[#2f6b3e] dark:text-[#9ed0a8] mb-2 truncate">{b?.name ?? 'B'}</div>
            <div className="flex flex-col gap-1.5">
              {metricsB.map((m) => {
                const aVal = byKey.get(m.key)
                return <MetricRow key={m.key} m={m} delta={aVal === undefined ? null : m.value - aVal} />
              })}
            </div>
          </div>
        </div>
