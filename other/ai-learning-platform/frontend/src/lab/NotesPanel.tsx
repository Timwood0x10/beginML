...
const UI: Record<"zh" | "en", Record<string, string>> = {
  ...
  en: {
    ...
    missingHint:
      'Could not find "{src}" in the note index — add it to the notes library to enable the jump.',
  },
};
...
export function NotesPanel({ notes }: { notes: LabNote[] }) {
  ...
  const open = (src: string) => {
    ...
    const suffix = pathToId
      ? [...pathToId.entries()].find(([p]) => p.endsWith(src))?.[1]
      : undefined;
    if (suffix) {
            ...
            >
              ...
              >
                ...
                <span className="flex-1 min-w-0">
                  <span className="block text-body-md text-on-surface dark:text-dark-on-surface truncate">
                    {n.title}
                  </span>
                  <span className="block text-caption text-outline font-mono truncate">
                    {n.src}
                  </span>
                </span>
              ...
              {isMissing && (
                <div className="border-t border-outline-variant/40 dark:border-white/10 px-4 py-3">
                  <div className="rounded-xl p-3 border border-dashed border-outline-variant/60 dark:border-white/15">
                    <div className="text-label-md font-semibold text-on-surface-variant dark:text-outline mb-1">
                      📄 {ui.missing}
                    </div>
                    <div className="text-caption text-on-surface-variant dark:text-outline">
                      {ui.missingHint.replace("{src}", n.src)}
                    </div>
                  </div>
                </div>
              )}
