...
function drawCurve(ctx: CanvasRenderingContext2D, s: Scale, xs: number[], ys: number[], color: string, width: number) {
...
function drawActivation(canvas: HTMLCanvasElement, r: ActResult, hoverX: number | null) {
  const W = 640, H = 420
...
export default function ActivationLab({ result, params, setParams }: {
  ...
  const clampX = (x: number) => {
    if (!r) return x
    const lo = r.domain.x[0], hi = r.domain.x[1]
    return Math.max(lo + 0.01, Math.min(hi - 0.01, x))
  }

  const { handlers } = useCanvasDrag({
    getScale: () => scaleRef.current,
    onDown: (x) => { setParams({ ...params, point: parseFloat(clampX(x).toFixed(2)) }); return true },
    onDrag: (x) => { setParams({ ...params, point: parseFloat(clampX(x).toFixed(2)) }) },
