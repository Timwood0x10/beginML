...
interface GdResult extends LabResult {
  ...
  trajectories: {
    name: string;
    color: string;
    points: { x: number; y: number; loss: number }[];
  }[];
...
function drawContour(
  ...
  for (let i = 0; i < x.length - 1; i++) {
    for (let j = 0; j < y.length - 1; j++) {
      ...
      ctx.fillRect(
        s.px(x[i]),
        s.py(y[j + 1]),
        s.px(x[i + 1]) - s.px(x[i]) + 0.5,
        s.py(y[j]) - s.py(y[j + 1]) + 0.5,
      );
    }
  }
  drawAxes(ctx, s, r.domain, {
    color: ink,
    gridColor: "rgba(125,118,109,0.15)",
  });
  ...
  ctx.beginPath();
  ctx.arc(s.px(r.minimum.x), s.py(r.minimum.y), 7, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = bg;
  ctx.lineWidth = 2;
  ctx.stroke();
  ...
  for (const traj of r.trajectories) {
    ...
    traj.points.forEach((p, i) => {
      if (i % Math.max(1, Math.floor(traj.points.length / 12)) === 0) {
        ctx.beginPath();
        ctx.arc(s.px(p.x), s.py(p.y), 2.2, 0, Math.PI * 2);
        ctx.fill();
      }
    });
    ...
    if (last) {
      ctx.beginPath();
      ctx.arc(s.px(last.x), s.py(last.y), 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = bg;
      ctx.lineWidth = 2;
      ctx.stroke();
...
export default function GradientDescentLab({
  ...
  const { handlers } = useCanvasDrag({
    ...
    onDown: (x, y) => {
      ...
      setParams({
        ...params,
        startX: parseFloat(p.x.toFixed(2)),
        startY: parseFloat(p.y.toFixed(2)),
      });
      return true;
    },
    onDrag: (x, y) => {
  ...
  if (!r)
    return (
      <div className="p-10 text-on-surface-variant dark:text-outline">
        {loading ? "Computing trajectories..." : "Adjust controls to begin."}
      </div>
    );
    ...
    <div className="flex flex-col gap-4">
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="w-full overflow-x-auto">
          <canvas
            ref={contourRef}
            className="rounded-2xl w-full cursor-crosshair touch-none"
            style={{ maxWidth: 560 }}
            {...handlers}
          />
        </div>
