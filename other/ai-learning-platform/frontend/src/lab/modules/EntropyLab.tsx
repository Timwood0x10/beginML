...
function BernoulliView({
  ...
  useEffect(() => {
    ...
    if (hover !== null && !inspect) {
      ...
      ctx.setLineDash([3, 3]);
      ctx.beginPath();
      ctx.moveTo(s.px(hover), s.py(r.domain.y[0]));
      ctx.lineTo(s.px(hover), s.py(r.domain.y[1]));
      ctx.stroke();
      ctx.setLineDash([]);
    }
    ...
    if (inspect) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(ix, s.py(r.domain.y[0]));
      ctx.lineTo(ix, s.py(r.domain.y[1]));
      ctx.stroke();
      ...
      pts.forEach(([v, color]) => {
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.arc(ix, s.py(v), 5, 0, Math.PI * 2);
        ctx.fill();
      });
    }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
          Click the chart to inspect H, KL and cross-entropy at any Q.
        </p>
      </div>
      ...
      {inspect && (
        <div className="bg-surface-container-low dark:bg-dark-surface rounded-2xl p-4 border border-outline-variant/40 dark:border-white/10 grid grid-cols-3 gap-3">
          <Stat
            label={`H(Q=${inspect.x.toFixed(2)})`}
            value={inspect.h.toFixed(3)}
          />
...
function CategoricalView({ r }: { r: CategoricalResult }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <FormulaCard
        formula={r.formula}
        title={`Categorical (k=${r.categories.length}, T=${r.temperature})`}
      />
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-6 border border-outline-variant/40 dark:border-white/10">
              ...
              >
                <div className="flex items-end gap-1 h-full w-full">
                  <div
                    ...
                    style={{ height: `${hp}%`, background: "#7A5C36" }}
                    title={`P=${r.p[i].toFixed(3)}`}
                  />
                  <div
                    className="flex-1 rounded-t-md transition-all"
                    style={{ height: `${hq}%`, background: "#C8604A" }}
                    title={`Q=${r.q[i].toFixed(3)}`}
                  />
                </div>
        ...
        <div className="flex gap-4 mt-4 text-caption">
          <span className="inline-flex items-center gap-1.5 text-on-surface-variant dark:text-outline">
            <span className="w-3 h-3 rounded-sm bg-[#7A5C36]" /> P (true)
          </span>
          <span className="inline-flex items-center gap-1.5 text-on-surface-variant dark:text-outline">
            <span className="w-3 h-3 rounded-sm bg-[#C8604A]" /> Q (model, T=
            {r.temperature})
          </span>
        </div>
      </div>
...
function Stat({
  ...
  return (
    ...
    >
      <div
        className={`font-mono text-xl font-bold ${highlight ? "text-primary dark:text-inverse-primary" : "text-primary dark:text-inverse-primary"}`}
      >
        {value}
      </div>
