...
function drawReg(
  ...
  for (const c of r.contacts) {
    ...
    ctx.beginPath();
    ctx.arc(s.px(px), s.py(py), 6, 0, Math.PI * 2);
    ctx.fill();
...
export default function RegularizationLab({
  ...
  const clamp = (v: number, lo: number, hi: number) =>
    Math.max(lo + 0.1, Math.min(hi - 0.1, v));
  ...
  const { handlers } = useCanvasDrag({
    ...
    onDown: (x, y) => {
      ...
      setParams({
        ...params,
        optX: parseFloat(clamp(x, 0.2, 2.8).toFixed(2)),
        optY: parseFloat(clamp(y, 0.2, 2.8).toFixed(2)),
      });
      return true;
    },
    onDrag: (x, y) => {
    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          <div className="flex flex-wrap gap-3 text-caption">
            ...
            {r.shapes.map((sh) => (
              <Legend
                key={sh.type}
                color={sh.type === "l1" ? "#C8604A" : "#5B6BB0"}
                label={`${sh.type.toUpperCase()} ball`}
                dashed={sh.type === "l2"}
              />
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {r.contacts.map((c) => (
          <div
            key={c.penalty}
            className="bg-surface-container-lowest dark:bg-dark-surface rounded-2xl p-5 border border-outline-variant/40 dark:border-white/10"
          >
            <div className="flex items-center justify-between mb-3">
              <h4
                className="font-label-md text-label-md uppercase tracking-wider inline-flex items-center gap-2"
