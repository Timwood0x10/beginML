  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/5">
            ...
            {(['weights', 'scores'] as View[]).map((v) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-caption font-semibold capitalize transition ${
                  view === v ? 'bg-primary text-on-primary dark:bg-inverse-primary dark:text-inverse-surface' : 'text-on-surface-variant dark:text-outline'
                }`}
              >
        ...
        <div className="w-full overflow-x-auto">
          <div className="inline-block min-w-full">
            ...
            <div className="flex ml-16">
              {r.tokens.map((t) => (
                <div key={t} className="w-12 md:w-14 text-center text-caption font-semibold text-on-surface-variant dark:text-outline pb-1">
                  {t}
                </div>
              ))}
              <div className="w-16" />
            ...
            <div className="flex items-center mt-2 ml-16">
              <span className="text-caption text-outline w-[calc(100%-4rem)]">← query attends over keys →</span>
              <span className="w-16 text-caption text-outline text-center">focus</span>
            </div>
          </div>
        </div>
        ...
        {hover && !((r.causal && hover.col > hover.row)) && (
          <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
            ...
            {' assigns weight '}
            <span className="font-mono font-semibold">{r.weights[hover.row][hover.col].toFixed(3)}</span>
            {' to '}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <VectorCard title="Query vector" name="Q" tokens={r.tokens} row={hover?.row} vectors={r.Q} d={r.d} />
        <VectorCard title="Key vector" name="K" tokens={r.tokens} row={hover?.col} vectors={r.K} d={r.d} />
      </div>
    </div>
  )
}
...
function VectorCard({
  title, name, tokens, row, vectors, d,
}: {
  title: string; name: string; tokens: string[]; row?: number; vectors: number[][]; d: number
}) {
  const idx = row ?? 0
    ...
    <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-2xl p-5 border border-outline-variant/40 dark:border-white/5">
      ...
      <div className="flex items-end gap-1 h-20">
        {Array.from({ length: d }).map((_, k) => {
          ...
          return (
            <div key={k} className="flex-1 flex flex-col justify-end" title={`${name}[${k}]=${v.toFixed(3)}`}>
              <div
                className="rounded-t"
                style={{
                  height: `${h}%`,
