...
export default function DangerousMountainLab({
  result,
  loading,
  params,
  onRecord,
  ...
  const l2UnderfitMin = r
    ? Math.min(...r.test.filter((_, i) => r.caps[i] < 0.75 * r.n))
    : Infinity;
  const l2Achieved =
    l2Goal === "valley"
      ? (r?.state ?? "") === "overparam" &&
        (r?.test_error ?? Infinity) < l2UnderfitMin
      : l2Goal === "peak"
  ...
  const underfitMin = Math.min(
    ...r.test.filter((_, i) => r.caps[i] < 0.75 * r.n),
  );
  const quests: Quest[] = [
    {
      id: "valley",
      label: texts.quests![0],
      done: r.state === "overparam" && r.test_error < underfitMin,
    },
    { id: "peak", label: texts.quests![1], done: r.capacity === r.peak_cap },
    {
      id: "min",
      label: texts.quests![2],
      done: r.test_error === Math.min(...r.test),
    },
  ];
  ...
  const px = (cap: number) =>
    pad.l + ((cap - capMin) / (capMax - capMin)) * plotW;
  const py = (err: number) => pad.t + (1 - Math.min(err, yMax) / yMax) * plotH;
  const poly = (vals: number[]) =>
    caps
      .map((c, i) => `${px(c).toFixed(1)},${py(vals[i]).toFixed(1)}`)
      .join(" ");
  ...
  const stateCard = {
    underfit: {
      icon: "🌱",
      title: texts.ui.stateUnderfit,
      desc: texts.ui.stateUnderfitDesc,
      cls: "border-[#5B6BB0]/40 text-[#5B6BB0] dark:text-[#aab4dd]",
    },
    danger: {
      icon: "⚠️",
      title: texts.ui.stateDanger,
      desc: texts.ui.stateDangerDesc,
      cls: "border-[#C8604A]/40 text-[#C8604A] dark:text-[#f0b3a4]",
    },
    overparam: {
      icon: "🏔️",
      title: texts.ui.stateOverparam,
      desc: texts.ui.stateOverparamDesc,
      cls: "border-[#2f6b3e]/40 text-[#2f6b3e] dark:text-[#9ed0a8]",
    },
  }[r.state];
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <svg
          ...
          className="w-full"
          role="img"
          aria-label="double descent"
        >
          {/* danger zone */}
          <rect
            x={px(dzLo)}
            y={pad.t}
            width={px(dzHi) - px(dzLo)}
            height={plotH}
            fill="rgba(200,96,74,0.10)"
            stroke="rgba(200,96,74,0.25)"
            strokeDasharray="3 3"
          />
          <text
            x={(px(dzLo) + px(dzHi)) / 2}
          ...
          <polyline
            points={poly(fused)}
            fill="none"
            stroke="rgba(91,107,176,0.55)"
            strokeWidth="2"
            strokeDasharray={blend < 0.98 ? "6 4" : "none"}
          />
          {/* train (green) */}
          <polyline
            points={poly(train)}
        ...
        <div className="flex flex-wrap gap-4 mt-3 text-caption text-on-surface-variant dark:text-outline">
          ...
          <span className="inline-flex items-center gap-1.5">
            <span className="w-4 h-0.5 rounded bg-[#2f6b3e]" />{" "}
            {texts.ui.trainError}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-4 border-t-2 border-dashed border-[#7d766d]" />{" "}
            {texts.ui.classicView}
          </span>
        </div>
      </div>
      ...
      {/* State card */}
      <div
        className={`rounded-3xl p-5 border ${stateCard.cls} bg-surface-container-lowest dark:bg-dark-surface`}
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-caption text-outline ml-auto font-mono">
            {fmt(texts.ui.verdictEvidence, {
              u: underMin.toFixed(2),
              p: r.peak_error.toFixed(2),
              o: overMin.toFixed(2),
            })}
          </span>
        </div>
        <p className="text-body-md text-on-surface dark:text-inverse-on-surface">
          {stateCard.desc}
        </p>
      </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-5 border border-outline-variant/40 dark:border-white/10">
        ...
        {submitted && answer !== null && (
          ...
          >
            ...
            <div className="mt-2 text-label-md font-semibold text-primary dark:text-inverse-primary">
              {texts.aha}
            </div>
          </div>
        )}
        ...
        {l2Goal && (
          <div>
            <div className="mb-2 text-caption text-on-surface-variant dark:text-outline">
              ...
              {" · "}test{" "}
              <span className="font-mono">{r.test_error.toFixed(4)}</span>
            </div>
            <div
