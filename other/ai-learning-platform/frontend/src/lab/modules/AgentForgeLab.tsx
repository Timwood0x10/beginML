...
interface ForgeRun {
  ...
  failures: { node: string; label: string; icon: string; mode: string; message: string; impact: string; recovery: string; recovered: boolean; needs: string | null }[]
  recovered: number
  unrecovered: number
  resilience: number
  status: string
...
interface ForgeResult extends LabResult {
  ...
  run: ForgeRun
  yaml: string
  validation: { valid: boolean; violations: { kind: string; from: string; to: string; port: string; expected: string[]; message: string }[] }
  compare: {
    active: boolean
  ...
  baseline?: { name: string; success: boolean; resilience: number; llm_calls: number; tokens: number; latency: number; recovery: number; failures: number }
  variant?: { name: string; success: boolean; resilience: number; llm_calls: number; tokens: number; latency: number; recovery: number; failures: number }
  // evolve mode payload
  history?: { gen: number; best: number; avg: number }[]
  best?: { fitness: number; graph: { name: string; nodes: { id: string }[]; edges: { from: string; to: string; port: string }[] } }
  fittest?: { name: string; fitness: number }[]
}

interface CanvasNode { id: string; type: string; x: number; y: number }
interface CanvasEdge { from: string; to: string; port: string }
...
export default function AgentForgeLab({
  ...
  const [drawer, setDrawer] = useState<null | 'trace' | 'inspect' | 'knowledge'>(null)
  const [playing, setPlaying] = useState(false)
  const [step, setStep] = useState(0)
  const [dragType, setDragType] = useState<string | null>(null)
  const [dragPos, setDragPos] = useState<{ x: number; y: number } | null>(null)
  const [wire, setWire] = useState<{ from: string; port: string; x: number; y: number } | null>(null)
  ...
  useEffect(() => {
    ...
    if (r.preset !== lastPreset.current) {
      ...
      setNodes(r.graph.nodes.map((n, i) => ({ id: n.id, type: n.id, x: gridX(i), y: gridY(i) })))
      setEdges(r.graph.edges.map((e) => ({ from: e.from, to: e.to, port: e.port })))
      setSelected(null)
    }
  ...
  const validation = useMemo(() => {
    const violations: { from: string; to: string; port: string; expected: string[]; message: string }[] = []
    for (const e of edges) {
      const dst = r?.primitives[e.to]
      if (!dst) continue
      if (!dst.ports.in.includes(e.port)) {
        violations.push({
          from: e.from, to: e.to, port: e.port,
  ...
  useEffect(() => {
    if (!playing || !r) return
    if (step >= r.run.trace.length) { setPlaying(false); return }
  ...
  const loadExample = (id: string) => {
    setParams({ ...params, preset: id, graph: undefined, runToken: Date.now() })
  }
  ...
  const onCanvasDrop = (e: React.DragEvent) => {
    ...
    addPrimitive(dragType, e.clientX - rect.left - NODE_W / 2, e.clientY - rect.top - NODE_H / 2)
    setDragType(null)
    setDragPos(null)
  }
  ...
  const moveNode = (id: string, originX: number, originY: number, dx: number, dy: number) => {
    ...
    setNodes((ns) => ns.map((n) => (n.id === id ? { ...n, x: originX + dx, y: originY + dy } : n)))
  }

  const startWire = (from: string, port: string, clientX: number, clientY: number) => {
    setWire({ from, port, x: clientX, y: clientY })
    const onMove = (ev: MouseEvent) => setWire((w) => (w ? { ...w, x: ev.clientX, y: ev.clientY } : w))
    const onUp = (ev: MouseEvent) => {
      window.removeEventListener('mousemove', onMove)
      window.removeEventListener('mouseup', onUp)
      // Hit-test: which node is under the cursor right now?
      const canvas = canvasRef.current
      if (!canvas) { setWire(null); return }
      const rect = canvas.getBoundingClientRect()
      const cx = ev.clientX - rect.left
      const cy = ev.clientY - rect.top
      const target = nodes.find((n) => cx >= n.x && cx <= n.x + NODE_W && cy >= n.y && cy <= n.y + NODE_H)
  ...
  const endWire = (to: string) => {
    if (!wire) return
    if (!to) { setWire(null); return }
    const dst = r.primitives[to]
    if (!dst) { setWire(null); return }
    if (dst.ports.in.includes(wire.port)) {
      ...
      setMismatch(`${r.primitives[wire.from]?.label}.${wire.port} → ${dst.label} expects [${dst.ports.in.join(', ')}]`)
  ...
  const drawerContent = () => {
    ...
    if (drawer === 'inspect') return <InspectDrawer meta={selected ? r.primitives[selected] : null} id={selected} />
    if (drawer === 'knowledge') return <KnowledgeDrawer primitives={r.primitives} groups={r.groups} />
    return null
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['build', 'run', 'experiment'] as const).map((m) => (
          <button
            key={m}
            onClick={() => { setMode(m); if (m !== 'run') { setPlaying(false); setStep(0) } }}
        ...
        {mode === 'run' && (
          <button
            onClick={() => { if (playing) { setPlaying(false); setStep(r.run.trace.length) } else { setStep(0); setPlaying(true) } }}
            className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-primary text-on-primary dark:bg-inverse-primary dark:text-inverse-surface font-label-md text-label-md hover:opacity-90 transition"
          >
            <span className="material-symbols-outlined" style={{ fontSize: 17 }}>{playing ? 'stop' : 'play_arrow'}</span>
        ...
        <div className="ml-auto flex items-center gap-2">
          {mode === 'build' && (
            <span className={`inline-flex items-center gap-1.5 text-caption font-semibold ${validation.valid ? 'text-[#3A6B58] dark:text-[#a8d3dc]' : 'text-[#C8604A]'}`}>
              <span className={`w-2 h-2 rounded-full ${validation.valid ? 'bg-[#3A6B58]' : 'bg-[#C8604A]'}`} />
              {validation.valid ? 'Agent valid' : `${validation.violations.length} contract violation${validation.violations.length > 1 ? 's' : ''}`}
            </span>
          )}
          {mode === 'run' && (
      ...
      <div className="flex gap-4">
        ...
        <aside className="w-52 shrink-0 flex flex-col gap-4 max-h-[calc(100vh-12rem)] overflow-y-auto pr-1">
          <div className="rounded-2xl bg-surface-container-lowest dark:bg-dark-surface border border-outline-variant/40 dark:border-white/10 p-3">
            <h4 className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-2">Examples</h4>
            <div className="flex flex-col gap-1">
              {r.examples.map((ex) => (
                ...
                >
                  ...
                  <span className="block text-[10px] opacity-75 truncate">{ex.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-surface-container-lowest dark:bg-dark-surface border border-outline-variant/40 dark:border-white/10 p-3">
            ...
            <div className="flex flex-col gap-2.5">
              {(['brain', 'memory', 'action', 'capability', 'recovery', 'agent'] as const).map((cls) => {
                ...
                const bricks = isRecovery
                  ? Object.entries(r.recoveryBricks).map(([id, b]) => ({ id, label: b.label, icon: b.icon, desc: b.desc }))
                  : Object.entries(r.primitives)
                      .filter(([, p]) => p.class === cls)
                      .map(([id, p]) => ({ id, label: p.label, icon: p.icon, desc: p.desc }))
                if (bricks.length === 0) return null
                return (
                  <div key={cls}>
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="w-2 h-2 rounded-full" style={{ background: clsMeta.color }} />
                      <span className="text-[9px] uppercase tracking-wider font-semibold text-on-surface-variant dark:text-outline">
                        {clsMeta.label}
                      </span>
                      <span className="text-[8px] px-1 rounded bg-surface-variant dark:bg-white/10 text-outline">{clsMeta.rarity}</span>
                    </div>
                          ...
                          <div
                            ...
                            onClick={() => {
                              if (isRecovery) {
                                ...
                                setRecoveries((rs) => attached ? rs.filter((r) => r !== b.id) : [...rs, b.id])
                                return
                              }
                            ...
                            title={b.desc}
                            style={selected === b.id || attached ? undefined : { borderLeft: `3px solid ${clsMeta.color}` }}
                          >
                            ...
                            {b.label}
                            {isRecovery && attached && (
                              <span className="ml-auto text-[8px] uppercase tracking-wider opacity-80">attached</span>
                            )}
                          </div>
                        )
                      })}
          ...
          <div className="rounded-2xl bg-surface-container-lowest dark:bg-dark-surface border border-outline-variant/40 dark:border-white/10 p-3">
            <h4 className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-2">
              Skill boxes <span className="normal-case">(a brick of bricks)</span>
            </h4>
            <div className="flex flex-col gap-1">
              {r.skillBoxes.map((sk) => (
                <div key={sk.id} className="rounded-lg border border-outline-variant/40 dark:border-white/10">
                  <button
                    onClick={() => setOpenSkill(openSkill === sk.id ? null : sk.id)}
                  ...
                  >
                    <span className="material-symbols-outlined" style={{ fontSize: 15, color: '#7A5C8E' }}>{sk.icon}</span>
                    <span className="font-semibold flex-1">{sk.label}</span>
                    <span className="material-symbols-outlined text-outline" style={{ fontSize: 13 }}>
                      {openSkill === sk.id ? 'expand_less' : 'expand_more'}
                    </span>
                  </button>
                  {openSkill === sk.id && (
                    <div className="px-2 pb-2">
                      <div className="text-[10px] text-outline mb-1.5">{sk.desc}</div>
                      <div className="flex flex-col gap-0.5">
                        {sk.inner.map((t) => (
                          <div key={t.id} className="flex items-center gap-1.5 px-1.5 py-0.5 text-caption text-on-surface-variant dark:text-outline">
          ...
          <div className="rounded-2xl bg-surface-container-lowest dark:bg-dark-surface border border-outline-variant/40 dark:border-white/10 p-3">
            <h4 className="text-[10px] uppercase tracking-wider text-[#C8604A] font-semibold mb-2">
              💥 Break it <span className="normal-case">(drag onto a brick)</span>
            </h4>
            <div className="flex flex-col gap-1">
              {r.breakBricks.map((b) => (
                <div
        ...
        >
          <div className="absolute inset-0 pointer-events-none" style={{
            backgroundImage: 'radial-gradient(rgba(140,122,107,0.14) 1px, transparent 1px)',
          ...
          <div className="absolute inset-0">
                ...
                return (
                  <g key={i}>
                    <path d={`M${x1},${y1} C${mx},${y1} ${mx},${y2} ${x2},${y2}`} fill="none"
                      stroke={active ? '#A8382A' : 'rgba(140,122,107,0.4)'} strokeWidth={active ? 2 : 1.2} />
                    <circle cx={x2 - 7} cy={y2} r="3" fill={active ? '#A8382A' : 'rgba(140,122,107,0.5)'} />
                  </g>
                )
              })}
              {wire && (() => {
                ...
                return <path d={`M${x1},${y1} C${mx},${y1} ${mx},${wire.y} ${wire.x},${wire.y}`} fill="none" stroke="rgba(200,96,74,0.7)" strokeWidth="1.6" strokeDasharray="5 4" />
              })()}
            </svg>
            ...
            {nodes.map((n) => {
              ...
              const breakMeta = broken ? r.breakBricks.find((b) => b.id === broken) : null
              return (
                <div
                  key={n.id}
                  onMouseDown={(e) => {
                    const sx = e.clientX, sy = e.clientY, nx = n.x, ny = n.y
                    const onMove = (ev: MouseEvent) => moveNode(n.id, nx, ny, ev.clientX - sx, ev.clientY - sy)
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-caption text-on-surface dark:text-dark-on-surface inline-flex items-center gap-1">
                      <span className="material-symbols-outlined" style={{ fontSize: 14, color: failed || broken ? '#C8604A' : undefined }}>
                        {failed || broken ? 'error' : meta?.icon ?? 'crop_square'}
                      </span>
                      {meta?.label ?? n.type}
                    </span>
                    <button
                  ...
                  {broken && breakMeta && (
                    <div
                      ...
                      onClick={(e) => { e.stopPropagation(); setBreaks((b) => { const nb = { ...b }; delete nb[n.id]; return nb }) }}
                    >
                  ...
                  {stepInfo && executed && (
                    <div className="mt-0.5 text-[10px] font-mono text-outline">
                      {stepInfo.latency}ms{stepInfo.tokens ? ` · ${stepInfo.tokens}t` : ''}
                    </div>
                  )}
                  {failed && failure && (
                    <div className="mt-0.5 flex items-center gap-1 text-[9px] max-w-full">
                      ...
                      <span className="inline-flex items-center gap-0.5 rounded bg-[#C8604A]/15 dark:bg-[#C8604A]/30 px-1 py-0.5 text-[#C8604A] font-semibold">
                        <span className="material-symbols-outlined" style={{ fontSize: 9 }}>bolt</span>
                      ...
                      {failure.recovered ? (
                        ...
                        <span className="inline-flex items-center gap-0.5 rounded bg-[#3A6B58]/10 dark:bg-[#3A6B58]/25 px-1 py-0.5 text-[#2f6068] dark:text-[#a8d3dc]">
                          <span className="material-symbols-outlined" style={{ fontSize: 9 }}>healing</span>
                  ...
                  <div className="mt-1.5 flex items-start justify-between">
                    <div className="flex flex-col gap-1">
                      {(meta?.ports.in ?? []).map((pt) => {
                        ...
                        const rejected = wire !== null && !meta?.ports.in.includes(wire.port)
                        return (
                          <span
                            key={pt}
                            className={`inline-flex items-center gap-1 ${
                              wire ? (canSnap ? 'forge-port-snap' : rejected ? 'forge-port-reject' : 'opacity-40') : ''
                            }`}
                            title={wire ? (canSnap ? `snap: accepts ${wire.port}` : rejected ? `cannot connect ${wire.port}` : undefined) : undefined}
                          >
                            <span
                              className={`w-2 h-2 rounded-full transition-all ${
                                canSnap ? 'ring-2 ring-[#3A6B58] scale-125' : rejected ? 'bg-[#C8604A]' : ''
                              }`}
                    ...
                    <div className="flex flex-col items-end gap-1">
                      {(meta?.ports.out ?? []).map((pt) => (
                        ...
                        >
                          <span className="text-[8px] text-outline">{pt}</span>
                          <span className="w-2 h-2 rounded-full" style={{ background: PORT_COLORS[pt] ?? '#8a8376' }} />
                        </span>
                      ))}
                    </div>
                  </div>
            ...
            {mismatch && (
              <div className="absolute left-1/2 -translate-x-1/2 bottom-4 z-20 rounded-xl bg-[#C8604A] text-white px-4 py-2.5 shadow-lg max-w-[90%]">
                <div className="text-[10px] uppercase tracking-wider opacity-80 font-semibold">Contract mismatch</div>
            ...
            {nodes.length === 0 && (
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center text-outline">
                  <div className="material-symbols-outlined" style={{ fontSize: 40 }}>add_circle</div>
                  <div className="text-caption mt-2">Drag primitives here to assemble your agent</div>
                </div>
              </div>
            )}
          </div>
      ...
      {mode === 'build' && !validation.valid && (
        <div className="rounded-xl bg-[#f3dfdc] dark:bg-[#3d2a28] text-[#8a3a35] dark:text-[#e9b8b2] px-3 py-2 text-caption">
          <div className="font-semibold mb-1">BUILD FAILED — {validation.violations.length} contract violation{validation.violations.length > 1 ? 's' : ''}</div>
          {validation.violations.map((v, i) => (
            <div key={i} className="font-mono text-[11px]">{v.message}</div>
          ))}
        </div>
      )}

      {/* EXPERIMENT mode */}
      {mode === 'experiment' && <ExperimentPanel result={r} params={params} setParams={setParams} />}
        ...
        <div className="rounded-2xl bg-surface-container-lowest dark:bg-dark-surface border border-outline-variant/40 dark:border-white/10 p-3 overflow-x-auto">
          <div className="flex items-center gap-0.5 flex-nowrap min-w-max">
              ...
              return (
                <span key={i} className="flex items-center gap-0.5 flex-nowrap">
                  ...
                  >
                    ...
                    {s.label}
                    {done && <span className="font-mono text-[9px] opacity-80">{s.latency}ms</span>}
                  </span>
                  {i < r.run.trace.length - 1 && (
                    <span className="text-outline mx-0.5">{s.failed ? '✗' : '→'}</span>
                  )}
                  {s.failed && failure && done && (
                    ...
                    >
                      ...
                      {failure.recovered ? failure.recovery : `no recovery (${(failure.needs ?? '?').replace('recovery-', '')})`}
                    </span>
                  )}
                </span>
              )
            ...
            {!playing && step >= r.run.trace.length && (
              <span className={`ml-2 text-caption font-semibold ${r.run.status === 'failed' ? 'text-[#C8604A]' : 'text-[#3A6B58]'}`}>
                {r.run.status === 'failed' ? '✗ agent down' : '✓ agent survived'} · resilience {r.run.resilience}
              </span>
            )}
          </div>
      ...
      {mode !== 'experiment' && (
        <>
          <div className="flex items-center gap-2">
            ...
            <span className="text-caption text-outline ml-auto">{nodes.length} nodes · {edges.length} contracts</span>
          </div>
          {drawer && (
...
function TraceDrawer({ run, step }: { run: ForgeRun; step: number }) {
  return (
    <div>
      <h4 className="font-label-md text-label-md uppercase tracking-wider text-on-surface dark:text-dark-on-surface mb-3">Run trace</h4>
      <div className="flex items-center gap-0 flex-wrap">
        {run.trace.map((s, i) => (
          <span key={i} className="flex items-center">
            <span className={`px-2 py-1 rounded-lg text-caption font-semibold inline-flex items-center gap-1 ${
              i < step ? 'bg-primary text-on-primary dark:bg-inverse-primary dark:text-inverse-surface' : 'bg-surface-container dark:bg-white/5 text-outline'
            }`}>
              <span className="material-symbols-outlined" style={{ fontSize: 13 }}>{s.icon}</span>
              {s.label}
              <span className="font-mono text-[9px] opacity-80">{s.latency}ms</span>
            </span>
            {i < run.trace.length - 1 && <span className="text-outline mx-1">→</span>}
          </span>
        ))}
      </div>
      ...
      {run.failures.length > 0 && (
        <div className="mt-2 rounded-xl bg-[#f3dfdc] dark:bg-[#3d2a28] text-[#8a3a35] dark:text-[#e9b8b2] px-3 py-2 text-caption">
          {run.failures.map((f, i) => (
            <div key={i} className="mb-1 last:mb-0"><b>{f.label}</b>: {f.recovery} <span className="opacity-70">({f.impact})</span></div>
          ))}
          <div className="mt-1 font-semibold">{run.status === 'failed' ? 'Agent down' : 'Agent survived'} · {run.recovered} recovery actions</div>
        </div>
      )}
    </div>
  )
...
function InspectDrawer({ meta, id }: { meta: PrimitiveMeta | null; id: string | null }) {
  if (!meta || !id) {
    return <div className="text-caption text-outline">Select a primitive to inspect its anatomy.</div>
  }
  return (
    <div>
      <h4 className="font-label-md text-label-md uppercase tracking-wider text-on-surface dark:text-dark-on-surface mb-2">{meta.label}</h4>
      ...
      <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="flex flex-col gap-0.5">
            {meta.tradeoffs.map((t, i) => <span key={i} className="text-caption text-on-surface-variant dark:text-outline">{t}</span>)}
          </div>
        </div>
      </div>
      <div className="mt-3 rounded-xl bg-surface-container dark:bg-white/5 px-3 py-2 inline-block">
        <div className="text-[10px] uppercase tracking-wider text-outline font-semibold">ARES note</div>
...
function KnowledgeDrawer({ primitives, groups }: { primitives: Record<string, PrimitiveMeta>; groups: string[] }) {
  return (
    <div>
      <h4 className="font-label-md text-label-md uppercase tracking-wider text-on-surface dark:text-dark-on-surface mb-3">Architecture evidence</h4>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {groups.map((g) =>
          Object.entries(primitives)
            .filter(([, p]) => p.group === g)
            .map(([id, p]) => (
              <div key={id} className="rounded-xl border border-outline-variant/40 dark:border-white/10 bg-surface-container dark:bg-white/5 p-3">
                <div className="text-[9px] uppercase tracking-wider text-outline font-semibold">{g}</div>
                <div className="font-semibold text-caption text-on-surface dark:text-dark-on-surface mt-0.5">{p.label}</div>
                <div className="text-[11px] text-on-surface-variant dark:text-outline mt-1">{p.why}</div>
                <div className="text-[10px] font-mono text-primary dark:text-inverse-primary mt-1.5">{p.note}</div>
              </div>
            )),
        )}
      </div>
...
function ExperimentPanel({ result, params, setParams }: {
  result: ForgeResult
  params: Record<string, unknown>
  setParams: (p: Record<string, unknown>) => void
}) {
  ...
  const [chaos, setChaos] = useState<Record<string, boolean>>({ chaos_mcp: true })
  ...
  const rows = [
    ['success', 'Success'], ['resilience', 'Resilience'], ['llm_calls', 'LLM calls'],
    ['tokens', 'Tokens'], ['latency', 'Latency (ms)'], ['recovery', 'Recovery'],
  ] as const
      ...
      {tab === 'compare' ? (
        <>
          ...
          <div className="flex flex-wrap items-end gap-4 mb-4">
            <div>
              <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1">Baseline</div>
              <select value={baseline} onChange={(e) => setBaseline(e.target.value)}
                ...
                {result.examples.map((ex) => <option key={ex.id} value={ex.id}>{ex.name}</option>)}
              </select>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1">Variant</div>
            ...
            <div className="flex flex-wrap gap-3">
              {(['chaos_memory', 'chaos_tool', 'chaos_mcp', 'chaos_llm', 'chaos_context'] as const).map((k) => (
                <label key={k} className="inline-flex items-center gap-1.5 text-caption text-on-surface-variant dark:text-outline cursor-pointer">
                  <input type="checkbox" checked={!!chaos[k]} onChange={(e) => setChaos((c) => ({ ...c, [k]: e.target.checked }))} className="accent-[#C8604A]" />
                  {k.replace('chaos_', '')}
                </label>
              ))}
            </div>
              ...
              <table className="w-full text-caption">
                <thead>
                  <tr className="text-outline text-left">
                    ...
                    <th className="pb-2 font-semibold text-right">{result.baseline.name}</th>
                    <th className="pb-2 font-semibold text-right">{result.variant.name}</th>
                ...
                <tbody>
                  {rows.map(([key, label]) => {
                    ...
                    const fmt = (v: unknown) => (typeof v === 'number' ? (Number.isInteger(v) ? v : v.toFixed(1)) : String(v))
                    return (
                      <tr key={key} className="border-t border-outline-variant/30 dark:border-white/10">
                        <td className="py-1.5 text-on-surface-variant dark:text-outline">{label}</td>
                        <td className="py-1.5 text-right font-mono">{fmt(a)}</td>
                        <td className={`py-1.5 text-right font-mono ${d && d < 0 ? 'text-[#3A6B58]' : d && d > 0 ? 'text-[#C8604A]' : 'text-on-surface dark:text-dark-on-surface'}`}>{fmt(b)}</td>
                        <td className="py-1.5 text-right font-mono text-outline">{d === null || d === 0 ? '—' : (d > 0 ? '+' : '') + fmt(Math.abs(d))}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <>
          ...
          <div className="flex flex-wrap items-end gap-4 mb-4">
            <div>
              <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1">Generations</div>
              <input type="number" min={2} max={10} value={generations}
            ...
            <div>
              <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1">Population</div>
              <input type="number" min={4} max={12} value={population}
                onChange={(e) => setPopulation(Number(e.target.value))}
                className="w-20 rounded-lg bg-surface-container dark:bg-white/5 border border-outline-variant/60 dark:border-white/10 px-3 py-1.5 text-caption text-on-surface dark:text-dark-on-surface" />
            </div>
            <button onClick={runEvolve}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-on-primary dark:bg-inverse-primary dark:text-inverse-surface font-label-md text-label-md hover:opacity-90 transition">
              <span className="material-symbols-outlined" style={{ fontSize: 16 }}>genetics</span>
              Run evolution
            </button>
          </div>
          ...
          {result.history && (
            <>
              ...
              <div className="mb-3">
                <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1.5">Fitness per generation</div>
                <div className="flex flex-col gap-1">
                  {result.history.map((h) => (
                    <div key={h.gen} className="flex items-center gap-2">
                      <span className="w-14 text-caption font-mono text-outline">G{h.gen}</span>
                      <div className="flex-1 h-3 bg-surface-variant dark:bg-white/10 rounded-full overflow-hidden">
                        <div className="h-full bg-primary dark:bg-inverse-primary" style={{ width: `${(h.best / maxFitness) * 100}%` }} />
                      </div>
                      <span className="w-20 text-caption font-mono text-on-surface dark:text-dark-on-surface text-right">
                        best {h.best} <span className="text-outline">/ avg {h.avg}</span>
                      </span>
                    </div>
                  ))}
                </div>
              ...
              {result.fittest && result.fittest.length > 0 && (
                <div className="mb-3">
                  <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1.5">Fittest</div>
                  <div className="flex flex-col gap-0.5">
                    {result.fittest.map((f, i) => (
                      <div key={i} className="flex items-center justify-between text-caption">
                        <span className="text-on-surface-variant dark:text-outline">#{i + 1} {f.name}</span>
                        <span className="font-mono text-primary dark:text-inverse-primary">{f.fitness}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {result.best && result.best.graph.nodes.length > 0 && (
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-outline font-semibold mb-1.5">🏆 Evolved agent</div>
                  <div className="flex flex-wrap gap-1.5">
                    {result.best.graph.nodes.map((n) => {
                      const meta = result.primitives[n.id]
                      return (
                        <span key={n.id} className="inline-flex items-center gap-1 rounded-lg bg-surface-container dark:bg-white/5 px-2 py-1 text-caption text-on-surface dark:text-dark-on-surface">
                          <span className="material-symbols-outlined" style={{ fontSize: 13 }}>{meta?.icon ?? 'crop_square'}</span>
                          {meta?.label ?? n.id}
                        </span>
                      )
                    })}
                  ...
                  <div className="mt-1 text-caption text-outline">
                    {result.best.graph.edges.length} contracts · fitness {result.best.fitness}
                  </div>
                </div>
              )}
            </>
