...
export default function MambaMemoryRaceLab({
  result,
  loading,
  params,
  onRecord,
  ...
  const lpy = (v: number) =>
    pad.t + (1 - Math.log(v / vMin) / Math.log(vMax / vMin)) * plotH;
  const poly = (vals: number[]) =>
    r.lengths
      .map((L, i) => `${lpx(L).toFixed(1)},${lpy(vals[i]).toFixed(1)}`)
      .join(" ");
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
          <div className="rounded-2xl px-4 py-3 border border-[#C8604A]/40 bg-[#C8604A]/10">
            <div className="text-caption text-[#C8604A] dark:text-[#f0b3a4] font-semibold mb-1">
              {texts.ui.transformer}
            </div>
            <div className="font-mono text-body-md text-on-surface dark:text-dark-on-surface">
              {texts.ui.lookback}: {r.walk_trans[walkIdx].toFixed(0)}{" "}
            ...
            <div className="text-caption text-on-surface-variant dark:text-outline mt-0.5">
              ...
              <span className="font-mono">
                {r.cum_trans[walkIdx].toFixed(0)}
              </span>
            </div>
          </div>
          <div className="rounded-2xl px-4 py-3 border border-[#2f6b3e]/40 bg-[#2f6b3e]/10">
            <div className="text-caption text-[#2f6b3e] dark:text-[#9ed0a8] font-semibold mb-1">
              {texts.ui.mamba}
            </div>
            <div className="font-mono text-body-md text-on-surface dark:text-dark-on-surface">
              {texts.ui.lookback}: {r.walk_mamba[walkIdx].toFixed(0)}{" "}
              <span className="text-caption text-outline">
                {texts.ui.fixedState}
              </span>
            </div>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-3">
            <span className="w-24 text-right text-caption font-semibold text-[#C8604A]">
              {texts.ui.transformer}
            </span>
            <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
              <div
                className="h-full rounded-md bg-[#C8604A] transition-all duration-300"
                style={{
                  width: `${(r.cum_trans[walkIdx] / Math.max(r.cum_trans[r.length - 1], 1)) * 100}%`,
                  opacity: 0.75,
                }}
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            <span className="w-24 text-right text-caption font-semibold text-[#2f6b3e]">
              {texts.ui.mamba}
            </span>
            <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
              <div
                className="h-full rounded-md bg-[#2f6b3e] transition-all duration-300"
                style={{
                  width: `${(r.cum_mamba[walkIdx] / Math.max(r.cum_trans[r.length - 1], 1)) * 100}%`,
                  opacity: 0.75,
                }}
              />
            </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        >
          ...
          {/* curves */}
          <polyline
            points={poly(r.transformer_flops)}
            fill="none"
            stroke="#C8604A"
            strokeWidth="2.4"
          />
          <polyline
            points={poly(r.mamba_flops)}
            fill="none"
            stroke="#2f6b3e"
            strokeWidth="2.4"
          />
          ...
          <line
            ...
            x2={lpx(r.length)}
            y2={pad.t + plotH}
            stroke="rgba(125,118,109,0.5)"
            strokeWidth="1.2"
            strokeDasharray="3 3"
          />
          <circle
            cx={lpx(r.length)}
            cy={lpy(r.transformer_flops_now)}
            r="4.5"
            fill="#C8604A"
            stroke="#fff"
            strokeWidth="1.5"
        ...
        <div className="mt-2 flex flex-wrap gap-4 text-caption text-on-surface-variant dark:text-outline">
          <span className="inline-flex items-center gap-1.5">
            <span className="w-4 h-0.5 rounded bg-[#C8604A]" />{" "}
            {texts.ui.transformer}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-4 h-0.5 rounded bg-[#2f6b3e]" /> {texts.ui.mamba}
          </span>
          {r.cross_point && (
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <span className="w-28 text-right text-caption font-semibold text-[#C8604A]">
              {texts.ui.transformer}
            </span>
            <div className="flex-1 h-5 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
              <div
                className="h-full rounded-md bg-[#C8604A] transition-all duration-300"
                style={{
                  width: `${(r.transformer_mem_now / memMax) * 100}%`,
                  opacity: 0.75,
                }}
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            <span className="w-28 text-right text-caption font-semibold text-[#2f6b3e]">
              {texts.ui.mamba}
            </span>
            <div className="flex-1 h-5 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
              <div
                className="h-full rounded-md bg-[#2f6b3e] transition-all duration-300"
                style={{
                  width: `${(r.mamba_mem_now / memMax) * 100}%`,
                  opacity: 0.75,
                }}
              />
            </div>
        ...
        <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
          {texts.ui.flops} @ L={r.length}：
          {texts.ui.verdictEvidence
            .replace("{t}", String(r.transformer_flops_now))
            .replace("{m}", String(r.mamba_flops_now))
            .replace("{r}", r.flops_ratio.toFixed(1))}
        </p>
      </div>
