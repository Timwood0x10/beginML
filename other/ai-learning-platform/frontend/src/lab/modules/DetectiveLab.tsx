...
export default function DetectiveLab({ result, loading }: {
  ...
  const correct = useMemo(() => {
    ...
    return answer === (top.type === 'feature' ? 'feature' : top.type === 'ffn' ? 'ffn' : 'head')
  }, [r, answer])
  ...
  const focusIdx = (trace && trace.length > 0 ? trace[trace.length - 1] : targetIdx) ?? targetIdx
  ...
  const headRow: { srcIdx: number; src: string; w: number }[] | null =
    suspect.type === 'head' && r.head_rows_full[suspect.name]
      ? Object.entries(r.head_rows_full[suspect.name][String(focusIdx)] ?? {})
          .map(([sj, w]) => ({ srcIdx: Number(sj), src: tokens[Number(sj)] ?? '?', w }))
          .sort((a, b) => b.w - a.w)
      : null
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <span className="text-caption text-outline font-mono">case: {r.case_id}</span>
        </div>
        ...
        <div className="text-caption uppercase tracking-wider font-semibold text-outline mb-2">{texts.ui.prediction}</div>
        <div className="flex flex-col gap-1.5">
          {r.prediction.map((p) => {
            const isTop = p.token === r.prediction[0].token
            return (
              <div key={p.token} className="flex items-center gap-3">
                <span className={`w-20 text-right text-caption font-semibold truncate ${isTop ? 'text-primary dark:text-inverse-primary' : 'text-on-surface dark:text-dark-on-surface'}`}>
                  {p.token}
                </span>
                <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
                  <div className="h-full rounded-md bg-primary dark:bg-inverse-primary transition-all duration-300"
                    style={{ width: `${(p.prob / maxProb) * 100}%`, opacity: isTop ? 0.9 : 0.45 }} />
                </div>
                <span className="w-12 text-right font-mono text-caption text-on-surface-variant dark:text-outline tabular-nums">{p.prob.toFixed(2)}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            {/* collection progress */}
            <span className={`inline-flex items-center gap-1.5 text-caption font-semibold px-2.5 py-1 rounded-full border ${
              allCollected
              ...
              {allCollected ? '✓ ' : ''}{fmt(texts.ui.progress, { a: collected.size, n: r.ranking.length })}
            </span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className={`text-label-md font-bold ${i === 0 ? 'text-[#C8604A]' : 'text-on-surface dark:text-dark-on-surface'}`}>
                      {i === 0 ? '🥇 ' : `${i + 1}. `}{s.name}
                    </span>
                    <span className="ml-auto font-mono text-caption text-outline">
                      {s.type === 'feature' ? texts.ui.feature : s.type === 'ffn' ? texts.ui.ffn : texts.ui.head}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-surface-container dark:bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-[#C8604A] transition-all duration-300"
                        style={{ width: `${(s.score / maxScore) * 100}%`, opacity: 0.6 + 0.4 * (s.score / maxScore) }} />
                    </div>
                    <span className="font-mono text-caption text-primary dark:text-inverse-primary">{s.score.toFixed(2)}</span>
                    <button
                      onClick={() => collect(i)}
                    ...
                    >
                      <span className="material-symbols-outlined" style={{ fontSize: 15 }}>{done ? 'check' : 'search'}</span>
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          ...
          <div className="text-caption text-outline mb-1 font-mono">{suspect.name} · {suspect.detail}</div>

          {/* trace breadcrumb */}
          <div className="flex flex-wrap items-center gap-1 mb-3 text-caption">
            {texts.ui.trace}: {(trace ?? [targetIdx]).map((ti, i) => (
              <span key={i} className="inline-flex items-center gap-1">
                {i > 0 && <span className="text-outline">→</span>}
                <button
                  onClick={() => setTrace((trace ?? [targetIdx]).slice(0, i + 1))}
                  className={`px-2 py-0.5 rounded-md font-mono ${
                    ti === targetIdx ? 'bg-primary/15 text-primary dark:text-inverse-primary' : 'bg-surface-container dark:bg-white/10 text-on-surface dark:text-dark-on-surface'
                  }`}
                >
                  {tokens[ti] ?? '?'}
                </button>
          ...
          {headRow ? (
            <div className="flex flex-col gap-1.5">
                ...
                <div key={srcIdx} className="flex items-center gap-3">
                  <button
                    ...
                    className={`w-16 text-right text-caption font-semibold truncate rounded transition ${
                      srcIdx === focusIdx ? 'text-[#2f6b3e] dark:text-[#9ed0a8]' : 'text-on-surface dark:text-dark-on-surface hover:text-primary dark:hover:text-inverse-primary'
                    }`}
                  ...
                  <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
                    <div className="h-full rounded-md bg-[#5B6BB0] transition-all duration-300"
                      style={{ width: `${Math.max(w * 100, 1)}%`, opacity: 0.35 + w * 0.65 }} />
                  </div>
              ...
              <p className="mt-2 text-caption text-on-surface-variant dark:text-outline">
                ← {tokens[focusIdx]} {texts.ui.attendsOver}（{texts.ui.traceHint2}）
              </p>
            </div>
          ) : suspect.type === 'feature' ? (
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap gap-1.5">
                {r.features.filter((f) => f.name === suspect.name).flatMap((f) => f.activated).map((tok, i) => (
                  <span key={i} className="px-2 py-1 rounded-lg bg-[#C8604A]/15 text-[#C8604A] dark:text-[#f0b3a4] text-caption font-semibold">
                    {tok}
                  </span>
                ))}
              </div>
            ...
            <p className="text-caption text-on-surface-variant dark:text-outline">{suspect.detail}</p>
          )}
        </div>
      </div>

      {/* CASE CLOSED — gated until all evidence is collected */}
      <div className={`rounded-3xl p-4 md:p-6 border transition-all ${
        allCollected ? 'border-[#8a3a35]/40 dark:border-white/10' : 'border-outline-variant/40 dark:border-white/10 opacity-80'
        ...
        {allCollected ? (
          <>
            <div className="mb-3 text-caption text-outline font-mono">
              {fmt(texts.ui.verdictEvidence, { s: top.name, sc: top.score.toFixed(2) })}
            </div>
            ...
            <div className="mt-3 text-caption text-on-surface-variant dark:text-outline">
              {texts.ui.yourEvidence}: {r.ranking.map((s, i) => collected.has(i) ? s.name : null).filter(Boolean).join(' · ')}
            </div>
          </>
        ) : (
          <p className="text-caption text-on-surface-variant dark:text-outline">
