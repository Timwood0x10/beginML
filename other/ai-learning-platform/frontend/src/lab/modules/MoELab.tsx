...
export default function MoELab({
  ...
  useEffect(() => {
    if (r && l2Goal && l2Achieved && recordedL2.current !== l2Goal) {
      ...
      onRecord?.({
        question:
          l2Goal === "unbalanced" ? texts.ui.l2Unbalanced : texts.ui.l2Balanced,
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <span className="text-caption text-on-surface-variant dark:text-outline">
            ...
            {" · "}
            {texts.ui.gate} T ={" "}
            <span className="font-mono">{r.temperature.toFixed(2)}</span>
          </span>
        </div>
              ...
              return (
                ...
                >
                  <circle
                    ...
                    stroke={i === sel ? "#fff" : "rgba(125,118,109,0.4)"}
                  ...
                  <text
                    ...
                    fill={
                      i === sel ? "#fff" : "var(--ailearn-on-surface, #3B3023)"
                    }
                  >
                    {t}
                  </text>
            ...
            {row.map((w, e) => {
              ...
              return (
                <line
                  key={e}
                  x1={52}
                  y1={26 + sel * (208 / Math.max(1, r.n_tokens - 1))}
                  x2={exX - 16}
                  y2={exY}
            ...
            {r.experts.map((ex, e) => {
              ...
              return (
                <div key={e} className="flex items-center gap-3">
                  <span
                    className="w-24 text-right text-caption font-semibold truncate"
                    style={{ color: EXPERT_COLORS[e % EXPERT_COLORS.length] }}
                  >
                    {ex.name}
                  </span>
                  <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        background: kept
                          ? EXPERT_COLORS[e % EXPERT_COLORS.length]
                  ...
                  <span className="w-10 text-right font-mono text-caption text-on-surface-variant dark:text-outline tabular-nums">
                    {w.toFixed(2)}
                  </span>
                </div>
              );
              ...
              <div key={e} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-4 bg-surface-container dark:bg-white/5 rounded-md overflow-hidden">
                  <div
                    className="h-full rounded-md transition-all duration-300"
                    style={{
                      width: `${(ex.load_pct / maxLoadPct) * 100}%`,
                      background: EXPERT_COLORS[e % EXPERT_COLORS.length],
                      opacity: 0.65,
                    }}
                  />
                </div>
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-caption text-outline ml-auto font-mono">
                {fmt(texts.ui.verdictEvidence, {
                  max: Math.max(...r.loads).toFixed(2),
                  min: Math.min(...r.loads).toFixed(2),
                })}
              </span>
            </div>
