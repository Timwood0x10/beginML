...
export default function TransformerMRILab({
  result,
  loading,
  params,
  setParams,
  ...
  useEffect(() => {
    if (r && l2Goal && l2Achieved && recordedL2.current !== l2Goal) {
      ...
      onRecord?.({
        question:
          l2Goal === "pathology" ? texts.ui.l2Pathology : texts.ui.l2Healthy,
  ...
  const color =
    CHANNEL_COLOR[r.scan as keyof typeof CHANNEL_COLOR] ?? CHANNEL_COLOR.loss;
  ...
  const quests: Quest[] = [
    {
      ...
      done: r.scan === "grad_norm" && !r.health.ok,
    },
    {
      id: "healthy",
      label: texts.quests![1],
      done: r.scan === "loss" && r.health.ok,
    },
    {
      id: "boundary",
      label: texts.quests![2],
      done: r.scan === "grad_norm" && r.current_layer >= 8,
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-4 text-caption text-on-surface-variant dark:text-outline">
          ...
          <span className="inline-flex items-center gap-1.5">
            <span
              className="w-3 h-3 rounded-sm"
              style={{ background: color.hi }}
            />{" "}
            {vMax.toFixed(2)}
          </span>
          <span className="ml-auto">
            ← step → · {texts.ui.heatmap} ({r.provenance})
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          ...
          <svg
            ...
            className="w-full"
            role="img"
            aria-label="layer curve"
          >
            <polyline
              points={r.layer_curve
                .map((v, i) => {
            ...
            />
            <circle
              cx={10 + (r.current_step / Math.max(1, nSteps - 1)) * 300}
              cy={10 + 100 - (r.layer_curve[r.current_step] / vMax) * 100}
              r="3.5"
              fill="#C8604A"
              stroke="#fff"
              strokeWidth="1"
      ...
      <div
        className={`rounded-3xl p-4 md:p-6 border ${
          r.health.ok
            ? "border-[#2f6b3e]/40 dark:border-white/10"
            : "border-[#C8604A]/40 dark:border-white/10"
      ...
      <ABPanel
        ...
        b={shotB}
        onSnapA={() =>
          setShotA(makeShot(`A: L${r.current_layer}@${r.current_step}`))
        }
        onSnapB={() =>
          setShotB(makeShot(`B: L${r.current_layer}@${r.current_step}`))
        }
        onSwap={() => {
          setShotA(shotB);
          setShotB(shotA);
