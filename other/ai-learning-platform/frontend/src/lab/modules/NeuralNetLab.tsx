...
function drawBoundary(ctx: CanvasRenderingContext2D, s: Scale, grid: NnResult['grid']) {
  const { x, y, z } = grid
...
function drawContourLine(ctx: CanvasRenderingContext2D, s: Scale, grid: NnResult['grid'], level: number) {
  const { x, y, z } = grid
...
function drawScene(canvas: HTMLCanvasElement, r: NnResult, hover: { x: number; y: number } | null) {
  const W = 560, H = 480
...
export default function NeuralNetLab({ params }: {
  ...
  const [customPoints, setCustomPoints] = useState<{ x: number; y: number; cls: number }[]>([])
  ...
  const recompute = useCallback(async (pts: { x: number; y: number; cls: number }[]) => {
    setComputing(true)
    try {
      const res = await api.lab.compute('neural-net', {
        dataset, hidden, lr, epochs,
  ...
  useEffect(() => { recompute(customPoints) }, [recompute, customPoints])
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h3 className="font-headline text-lg text-on-surface dark:text-inverse-on-surface inline-flex items-center gap-2 capitalize">
            ...
            {customPoints.length > 0 ? 'custom' : dataset} dataset — {r?.architecture ?? '2-?-1'} MLP
            {computing && <span className="w-3 h-3 rounded-full border-2 border-outline-variant border-t-primary animate-spin" />}
          </h3>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setCls(1)} className={`px-3 py-1.5 rounded-lg text-caption font-semibold border transition ${cls === 1 ? 'text-white border-transparent' : 'bg-surface-container dark:bg-white/5 text-on-surface-variant dark:text-outline border-outline-variant/60 dark:border-white/10'}`} style={cls === 1 ? { background: COLORS[1] } : undefined}>
              + class
            </button>
            <button onClick={() => setCls(0)} className={`px-3 py-1.5 rounded-lg text-caption font-semibold border transition ${cls === 0 ? 'text-white border-transparent' : 'bg-surface-container dark:bg-white/5 text-on-surface-variant dark:text-outline border-outline-variant/60 dark:border-white/10'}`} style={cls === 0 ? { background: COLORS[0] } : undefined}>
              − class
            </button>
            ...
            <button onClick={demo} className="px-3 py-1.5 rounded-lg text-caption font-semibold bg-surface-container dark:bg-white/5 text-on-surface-variant dark:text-outline border border-outline-variant/60 dark:border-white/10 hover:text-primary">generated</button>
        ...
        <div className="w-full overflow-x-auto flex justify-center">
          <canvas ref={boundaryRef} className="rounded-2xl cursor-crosshair touch-none" {...handlers} />
        </div>
