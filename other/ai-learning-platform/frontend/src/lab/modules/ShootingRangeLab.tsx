...
export default function ShootingRangeLab({ result, loading, params, setParams, onRecord }: {
  ...
  const correct = useMemo(() => {
    ...
    const dom = r.bias2 >= r.variance && r.bias2 >= r.noise2 ? 'bias'
      : r.variance >= r.noise2 ? 'variance' : 'noise'
    return answer === dom
  }, [r, answer])
  ...
  const cx = T / 2, cy = T / 2
  const spread = Math.max(Math.abs(r.true_at_star), Math.max(...r.shots.map((s) => Math.abs(s - r.true_at_star))), 0.3)
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          ...
          <div className="flex justify-center">
            <svg viewBox={`0 0 ${T} ${T}`} className="w-full max-w-[260px]" role="img" aria-label="target">
              {rings.map((f) => (
                <circle key={f} cx={cx} cy={cy} r={(T / 2 - 10) * f} fill="none"
                  stroke="rgba(125,118,109,0.25)" strokeWidth="1.5" strokeDasharray="3 4" />
              ))}
              {/* noise floor ring */}
              <circle cx={cx} cy={cy} r={Math.min((T / 2 - 10), Math.sqrt(r.noise2) * K)}
                fill="none" stroke="rgba(200,96,74,0.35)" strokeWidth="1" strokeDasharray="2 3" />
              {/* shots */}
              {shotPts.map((p, i) => (
                <circle key={i} cx={p.x} cy={p.y} r="3" fill="#C8604A" opacity="0.55" />
              ))}
              {/* mean shot marker */}
              <circle cx={cx + meanRad} cy={cy} r="5" fill="#2f6b3e" stroke="#fff" strokeWidth="1.5" />
              {/* bullseye */}
              <circle cx={cx} cy={cy} r="4" fill="#54483A" stroke="#fff" strokeWidth="1.5" />
          ...
          <div className="mt-3 flex flex-col gap-1 text-caption text-on-surface-variant dark:text-outline">
            <span>🎯 {texts.ui.target} = <span className="font-mono text-primary dark:text-inverse-primary">{r.true_at_star.toFixed(3)}</span></span>
            <span>🔴 {texts.ui.shots}</span>
            <span>🟢 {texts.ui.meanShot} = <span className="font-mono">{r.mean_shot.toFixed(3)}</span>（偏差 = {(r.mean_shot - r.true_at_star).toFixed(3)}）</span>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          ...
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="function">
            {/* fits band */}
            {r.fits.map((fit, i) => (
              <path key={i} d={fit.map((v, j) => `${j === 0 ? 'M' : 'L'}${px(r.x[j]).toFixed(1)},${py(v).toFixed(1)}`).join(' ')}
                fill="none" stroke="rgba(91,107,176,0.28)" strokeWidth="1.4" />
            ))}
            {/* data points */}
            {r.xs.map((v, i) => (
              <circle key={i} cx={px(v)} cy={py(r.ys[i])} r="2.5" fill="#C8604A" opacity="0.6" />
            ))}
            {/* true function */}
            <path d={truePath} fill="none" stroke="#2f6b3e" strokeWidth="2.4" strokeLinejoin="round" />
          </svg>
          <div className="mt-2 flex flex-wrap gap-3 text-caption text-on-surface-variant dark:text-outline">
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-0.5 rounded bg-[#2f6b3e]" /> {texts.ui.trueFn}</span>
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-0.5 rounded bg-[#C8604A]" /> {texts.ui.dataPoints}</span>
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-0.5 rounded bg-[#5B6BB0] opacity-60" /> {texts.ui.fits}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className={`rounded-3xl p-5 border bg-surface-container-lowest dark:bg-dark-surface ${
          r.quadrant === 'variance-dominated' ? 'border-[#C8604A]/40'
          : r.quadrant === 'bias-dominated' ? 'border-[#5B6BB0]/40'
          : r.quadrant === 'high-high' ? 'border-[#8a3a35]/40'
          : 'border-[#2f6b3e]/40'
        }`}>
          <h3 className="font-label-md text-label-md uppercase tracking-wider text-on-surface dark:text-dark-on-surface mb-2">📌 {texts.ui.quadrant}</h3>
          <div className="font-headline text-lg text-on-surface dark:text-inverse-on-surface">{quadLabel}</div>
          <div className="mt-2 font-mono text-caption text-on-surface-variant dark:text-outline">
            {fmt(texts.ui.verdictEvidence, { b: r.bias2.toFixed(3), v: r.variance.toFixed(3), n: r.noise2.toFixed(3) })}
          </div>
        </div>

        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-5 border border-outline-variant/40 dark:border-white/10">
          <h3 className="font-label-md text-label-md uppercase tracking-wider text-on-surface dark:text-dark-on-surface mb-3">🧮 {texts.ui.mse}</h3>
          <div className="h-5 w-full flex rounded-lg overflow-hidden">
            <div className="h-full bg-[#5B6BB0] transition-all duration-300" style={{ width: `${segBias}%` }} title={`Bias² = ${r.bias2.toFixed(4)}`} />
            <div className="h-full bg-[#C8604A] transition-all duration-300" style={{ width: `${segVar}%` }} title={`Variance = ${r.variance.toFixed(4)}`} />
            <div className="h-full bg-[#7A5C36] transition-all duration-300" style={{ width: `${segNoise}%` }} title={`Noise² = ${r.noise2.toFixed(4)}`} />
          </div>
          <div className="mt-3 flex flex-wrap gap-4 text-caption text-on-surface-variant dark:text-outline">
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-[#5B6BB0]" /> {texts.ui.bias2} = <span className="font-mono">{r.bias2.toFixed(3)}</span></span>
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-[#C8604A]" /> {texts.ui.variance} = <span className="font-mono">{r.variance.toFixed(3)}</span></span>
            <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-[#7A5C36]" /> {texts.ui.noise2} = <span className="font-mono">{r.noise2.toFixed(3)}</span></span>
            <span className="ml-auto inline-flex items-center gap-1.5 font-semibold">{texts.ui.mseTotal} = <span className="font-mono">{r.mse.toFixed(3)}</span></span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 border border-outline-variant/40 dark:border-white/10 flex flex-wrap items-center gap-3">
        ...
        <span className="text-body-md text-on-surface dark:text-inverse-on-surface">
          样本 {r.samples} → 500：<span className="font-mono text-primary dark:text-inverse-primary">variance {r.variance.toFixed(3)}</span>
        </span>
        <button
          ...
          <div className={`mt-4 rounded-2xl p-4 border ${correct ? 'border-[#2f6b3e]/40' : 'border-[#C8604A]/40'} bg-surface-container dark:bg-white/5`}>
            ...
            <ul className="text-caption text-on-surface-variant dark:text-outline space-y-1">
              {texts.explanation.map((line) => (
                <li key={line}>• {fmt(line, { b: r.bias2.toFixed(3), v: r.variance.toFixed(3), n: r.noise2.toFixed(3) })}</li>
              ))}
            </ul>
          </div>
        )}
