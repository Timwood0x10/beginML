...
export default function WeightFreezerLab({ result, loading, onAction, params, onRecord }: {
  ...
  const correct = useMemo(() => {
    if (!r) return false
    const lossless = r.levels.filter((l) => l.label !== 'fp32' && l.error < 0.001)
    const target = lossless.length ? lossless[lossless.length - 1].label : 'ternary'
  ...
  // --- Pareto curve ------------------------------------------------------
  const PW = 420, PH = 220
  const plo = 30, pt = 14, pr = 12, pb = 26
  const bitsMax = 34
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <h3 className="font-headline text-lg text-on-surface dark:text-inverse-on-surface inline-flex items-center gap-2">
            <span className="material-symbols-outlined" style={{ fontSize: 20 }}>ac_unit</span>
            {texts.ui.title}
          </h3>
          <div className="inline-flex items-center gap-3 text-caption text-on-surface-variant dark:text-outline">
            <span>{texts.ui.current}: <span className="font-mono text-primary dark:text-inverse-primary">{r.current_label}</span></span>
            <span>{texts.ui.memory}: <span className="font-mono">{r.current_bits} bit</span></span>
            <span>{texts.ui.error}: <span className="font-mono">{(r.current_error * 100).toFixed(2)}%</span></span>
          </div>
        </div>

        <div className="flex justify-center">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[420px]" role="img" aria-label="weight cloud">
            {/* FP32 cloud (faint) */}
            {r.points.map((p, i) => (
              <circle key={`o${i}`} cx={px(p[0])} cy={py(p[1])} r="2.6" fill="#7d766d" opacity="0.22" />
            ))}
            {/* quantized cloud (solid) */}
            {r.quantized.map((p, i) => (
              <circle key={`q${i}`} cx={px(p[0])} cy={py(p[1])} r="2.8" fill={r.ternary ? '#8a3a35' : '#5B6BB0'} opacity="0.8" />
            ))}
          </svg>
        </div>
        <div className="mt-2 flex flex-wrap gap-4 text-caption text-on-surface-variant dark:text-outline">
          <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#7d766d] opacity-40" /> {texts.ui.fp32}</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#5B6BB0]" /> {texts.ui.quantized}</span>
          {r.ternary && <span className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#8a3a35]" /> {texts.ui.ternary}</span>}
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="flex flex-col gap-2">
          {staircase.map((label, i) => {
            ...
            return (
              <div key={label} className={`flex items-center gap-3 rounded-xl px-4 py-2.5 border transition-all ${
                active
                ...
                <span className="w-14 font-mono text-label-md font-bold text-on-surface dark:text-dark-on-surface">{label}</span>
                <div className="flex-1 h-4 bg-surface-container dark:bg-white/10 rounded-md overflow-hidden">
                  <div
                    className="h-full rounded-md transition-all duration-300"
                    style={{ width: `${Math.min(100, (lv.bits / 32) * 100)}%`, background: LEVEL_COLORS[i % LEVEL_COLORS.length], opacity: 0.65 }}
                  />
                </div>
                <span className="w-24 text-right font-mono text-caption text-on-surface-variant dark:text-outline tabular-nums">
                  {lv.bits} bit
                </span>
                <span className="w-28 text-right font-mono text-caption tabular-nums"
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <svg viewBox={`0 0 ${PW} ${PH}`} className="w-full" role="img" aria-label="pareto">
          {/* y grid (log) */}
          {[1, 0.1, 0.01, 0.001, 0.0001].map((v) => (
            <g key={v}>
              <line x1={plo} y1={lpy(v)} x2={PW - pr} y2={lpy(v)} stroke="rgba(125,118,109,0.14)" strokeDasharray="2 4" />
              <text x={plo - 6} y={lpy(v) + 3} textAnchor="end" fontSize="9" className="fill-outline">{v}</text>
            </g>
          ))}
          {/* curve */}
          <polyline
            points={r.levels.map((l) => `${lpx(l.bits).toFixed(1)},${lpy(l.error).toFixed(1)}`).join(' ')}
            fill="none" stroke="#7A5C36" strokeWidth="2"
          />
          {/* points */}
          {r.levels.map((l, i) => (
            <g key={l.label}>
              <circle cx={lpx(l.bits)} cy={lpy(l.error)} r={l.label === r.current_label ? 5 : 3.5}
                fill={l.label === r.current_label ? '#C8604A' : '#7A5C36'} stroke="#fff" strokeWidth="1.2" />
              <text x={lpx(l.bits)} y={lpy(l.error) - 8} textAnchor="middle" fontSize="9" className="fill-outline">{l.label}</text>
            </g>
          ))}
          {/* sweet spot band: INT4..INT8 */}
          <rect x={lpx(8)} y={pt} width={lpx(4) - lpx(8)} height={PH - pt - pb}
            fill="rgba(47,107,62,0.10)" stroke="rgba(47,107,62,0.3)" strokeDasharray="3 3" />
          <text x={(lpx(8) + lpx(4)) / 2} y={pt + 12} textAnchor="middle" fontSize="9" fill="#2f6b3e">
            {texts.ui.sweetSpot}
          </text>
          <text x={plo} y={PH - 8} fontSize="9" className="fill-outline">{texts.ui.memory} (bit) →</text>
          <text x={plo - 34} y={(pt + PH - pb) / 2} fontSize="9" className="fill-outline" transform={`rotate(-90 ${plo - 30} ${(pt + PH - pb) / 2})`}>
            {texts.ui.error}
          </text>
        </svg>
      </div>
