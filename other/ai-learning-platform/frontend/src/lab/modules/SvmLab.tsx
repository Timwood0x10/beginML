  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      drawContour(
        ...
        data.grid,
        1,
        dark ? "rgba(245,240,230,0.4)" : "rgba(29,28,22,0.35)",
        1,
      );
      drawContour(
        ctx,
    ...
    ctx.strokeStyle = dark
      ...
      : "rgba(125,118,109,0.15)";
    ctx.lineWidth = 1;
    if (domain.x[0] <= 0 && 0 <= domain.x[1]) {
      ctx.beginPath();
      ctx.moveTo(s.px(0), s.py(domain.y[0]));
      ctx.lineTo(s.px(0), s.py(domain.y[1]));
      ctx.stroke();
    }
    if (domain.y[0] <= 0 && 0 <= domain.y[1]) {
      ctx.beginPath();
      ctx.moveTo(s.px(domain.x[0]), s.py(0));
      ctx.lineTo(s.px(domain.x[1]), s.py(0));
      ctx.stroke();
    }
    ...
    pts.forEach((p) => {
      ...
      ctx.beginPath();
      ctx.arc(s.px(p.x), s.py(p.y), p.support ? 8 : 6, 0, Math.PI * 2);
      ctx.fill();
      if (p.support) {
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(s.px(p.x), s.py(p.y), 11, 0, Math.PI * 2);
        ctx.stroke();
      }
    });
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h3 className="font-headline text-lg text-on-surface dark:text-inverse-on-surface inline-flex items-center gap-2">
            ...
            Decision boundary
            {computing && (
              <span className="w-3 h-3 rounded-full border-2 border-outline-variant border-t-primary animate-spin ml-1" />
            )}
          </h3>
        ...
        <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
          Click on the canvas to place a point of the selected class. The SVM
          fits instantly; rings mark <strong>support vectors</strong>. Use a
          kernel from the sidebar for non-linear boundaries.
        </p>
      </div>
...
function Stat({
  ...
  return (
    <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-2xl p-4 border border-outline-variant/40 dark:border-white/10 text-center">
      <div
        className={`font-headline text-2xl font-bold text-primary dark:text-inverse-primary ${capitalize ? "capitalize" : ""}`}
      >
        {value}
      </div>
