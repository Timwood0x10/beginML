...
import { labTextsZh, labTextsEn } from "../../i18n/lab";
...
export default function FeatureHuntLab({
  result,
  loading,
  params,
  onRecord,
  ...
  const quests: Quest[] = [
    {
      ...
      done: verdict !== null && verdict.precision === 1 && verdict.recall === 1,
    },
    {
      id: "nofp",
      label: texts.quests![1],
      done: verdict !== null && verdict.precision === 1,
    },
    {
      id: "recall",
      label: texts.quests![2],
      done: verdict !== null && verdict.recall === 1,
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          ...
          <span className="text-caption text-on-surface-variant dark:text-outline">
            {r.n_features} {texts.ui.neuron} · {r.n_true} {texts.ui.isTrue} ·
            seed {r.seed}
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            <span
              className={`px-2.5 py-1 rounded-full text-caption font-bold ${
                selFeat.is_true
                  ? "bg-[#2f6b3e]/15 text-[#2f6b3e] dark:text-[#9ed0a8]"
                  : "bg-[#C8604A]/15 text-[#C8604A] dark:text-[#f0b3a4]"
              }`}
            >
          ...
          <div className="grid grid-cols-3 gap-2 mb-3 text-caption text-on-surface-variant dark:text-outline">
            <div className="rounded-xl bg-surface-container dark:bg-white/5 px-3 py-2">
              ...
              <div className="font-mono text-primary dark:text-inverse-primary">
                {selFeat.max.toFixed(3)}
              </div>
            </div>
            <div className="rounded-xl bg-surface-container dark:bg-white/5 px-3 py-2">
            ...
            <div className="rounded-xl bg-surface-container dark:bg-white/5 px-3 py-2">
              ...
              <div className="font-mono">
                {(selFeat.sparsity * 100).toFixed(0)}%
              </div>
            </div>
          </div>

          <div className="text-caption text-on-surface-variant dark:text-outline mb-2">
            {texts.ui.activeOn}:
          </div>
          <div className="flex flex-wrap gap-1.5">
            {r.tokens.map((t, ti) => {
              ...
              return (
                <span
                  key={ti}
                  className={`px-2 py-1 rounded-lg text-caption font-semibold border ${
                    isActive
                ...
                >
                  {t}{" "}
                  <span className="font-mono opacity-70">{v.toFixed(2)}</span>
                </span>
              );
            })}
          </div>
                ...
                <span
                  className={`font-label-md font-bold ${
                    verdict.precision >= 0.75
                      ...
                      : "text-[#C8604A] dark:text-[#f0b3a4]"
                  }`}
                >
                  {texts.ui.precision}: {(verdict.precision * 100).toFixed(0)}%
                  · {texts.ui.recall}: {(verdict.recall * 100).toFixed(0)}%
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {r.features
                  ...
                  .map((f) => (
                    <span
                      key={f.idx}
                      className="px-2 py-1 rounded-lg bg-[#2f6b3e]/15 text-[#2f6b3e] dark:text-[#9ed0a8] text-caption font-semibold"
                    >
          ...
          onClick={() => {
            ...
            if (onRecord && r) {
              ...
              onRecord({
                ...
                prediction: label,
                correct,
                evidence: `noise max=${r.features
                  .filter((f) => !f.is_true)
                  .reduce((a, b) => Math.max(a, b.max), 0)
                  .toFixed(2)}`,
                params: { ...params },
              });
            }
      ...
      <ExplainBox
        onRecord={onRecord}
        evidence={`true=${r.true_idx.join(",")}, noise max=${r.features
          .filter((f) => !f.is_true)
          .reduce((a, b) => Math.max(a, b.max), 0)
          .toFixed(2)}`}
        params={params}
