...
export default function TokenSocietyLab({
  result,
  loading,
  onAction,
  params,
  ...
  const correct = useMemo(() => {
    ...
    const ans =
      scout.name === "The Long-Distance Scout"
        ? "scout"
        : scout.name === "The Repeater"
          ? "repeater"
          : "nearby";
    return answer === ans;
  }, [r, answer]);
  ...
  const selTokenIdx =
    selToken === null ? Math.floor((r?.n ?? 2) / 2) : selToken;
  const currentMaxW =
    r && r.heads[Math.min(headIdx, r.heads.length - 1)]
      ? Math.max(
          ...r.heads[Math.min(headIdx, r.heads.length - 1)].weights[
            selTokenIdx
          ],
        )
      : 0;
  ...
  const quests: Quest[] = [
    {
      ...
      done: head.name === "The Long-Distance Scout",
    },
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
              ...
              return (
                ...
                >
                  <circle
                    ...
                    fill={
                      isSel
                        ? HEAD_COLORS[head.head % HEAD_COLORS.length]
                        : "rgba(125,118,109,0.18)"
                    }
        ...
        <p className="mt-2 text-caption text-on-surface-variant dark:text-outline">
          {texts.ui.clickToken} ·{" "}
          <span className="font-mono">{r.tokens[sel]}</span>
        </p>
      </div>
          ...
          {r.tokens.map((t, j) => {
            ...
            return (
              <div key={j} className="flex items-center gap-3">
                <span className="w-16 text-right text-caption font-semibold truncate text-on-surface dark:text-dark-on-surface">
                  {t}
                </span>
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    ...
                    style={{ background: HEAD_COLORS[i % HEAD_COLORS.length] }}
                  />
                  <span className="font-label-md font-bold text-on-surface dark:text-dark-on-surface">
                    {h.name}
                  </span>
                ...
                <div className="flex flex-wrap gap-3 text-caption text-on-surface-variant dark:text-outline">
                  <span>
                    {texts.ui.avgDist}{" "}
                    <span className="font-mono text-primary dark:text-inverse-primary">
                      {h.avg_dist.toFixed(2)}
                    </span>
                  </span>
                  <span>
                    {texts.ui.localRatio}{" "}
                    <span className="font-mono">
                      {h.local_ratio.toFixed(2)}
                    </span>
                  </span>
                  <span>
                    {texts.ui.diagRatio}{" "}
                    <span className="font-mono">{h.diag_ratio.toFixed(2)}</span>
                  </span>
                </div>
              </button>
            );
          ...
          onClick={() => {
            ...
            if (onRecord && r) {
              const label =
                ...
                answer ??
                "";
              const scout = r.heads.reduce((a, b) =>
                b.avg_dist > a.avg_dist ? b : a,
              );
              onRecord({
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-caption text-outline ml-auto font-mono">
                {fmt(texts.ui.verdictEvidence, {
                  d: r.heads
                    .reduce((a, b) => (b.avg_dist > a.avg_dist ? b : a))
                    .avg_dist.toFixed(2),
                })}
              </span>
            </div>
