...
function drawDist(canvas: HTMLCanvasElement, r: DistResult, hoverX: number | null) {
  const W = 640, H = 420
  ...
  if (hoverX !== null) {
    ...
    const cx = s.px(sm.x), cy = s.py(sm.y)
    ctx.strokeStyle = 'rgba(99,91,79,0.45)'
    ctx.setLineDash([3, 3])
    ctx.beginPath(); ctx.moveTo(cx, s.py(r.domain.y[0])); ctx.lineTo(cx, s.py(r.domain.y[1])); ctx.stroke()
    ctx.setLineDash([])
...
export default function DistributionLab({ result }: {
  ...
  const { handlers } = useCanvasDrag({
    ...
    onDown: () => true,
    onDrag: () => {},
    onHover: (x) => { if (r && !Number.isNaN(x)) setHoverX(Math.max(r.domain.x[0], Math.min(r.domain.x[1], x))) },
    onUp: () => {},
  })
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h3 className="font-headline text-lg text-on-surface dark:text-inverse-on-surface inline-flex items-center gap-2">
            ...
            {r.name}
            <span className="text-caption font-normal normal-case text-outline ml-2">(hover to inspect)</span>
          </h3>
        ...
        {sm && (
          <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
            {r.discrete ? 'P' : 'f'}({sm.x.toFixed(2)}) = <span className="font-mono text-primary dark:text-inverse-primary">{sm.y.toFixed(4)}</span>
          </p>
        )}
      </div>
