...
export default function RepresentationRiverLab({
  result,
  loading,
  params,
  onRecord,
  ...
  const quests: Quest[] = [
    {
      ...
      done: lastInj.attention < 0.3 && lastInj.ffn < 0.3,
    },
    {
      id: "maxattn",
      label: texts.quests![1],
      done: lastInj.attention >= 0.8 || lastInj.ffn >= 0.8,
    },
    {
      id: "both",
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-3xl px-5 py-3 border border-[#8a3a35]/40 bg-[#f3dfdc]/40 dark:bg-[#3d2a28]/40 flex items-center gap-3">
        <span
          className="material-symbols-outlined text-[#8a3a35]"
          style={{ fontSize: 20 }}
        >
          science
        </span>
        <div>
          <div className="text-label-md font-bold text-[#8a3a35]">
            {texts.ui.simulationMode}
          </div>
          <div className="text-caption text-on-surface-variant dark:text-outline">
            {texts.ui.simulationHint}
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <svg
          ...
          className="w-full"
          role="img"
          aria-label="representation river"
        >
          {/* layer gridlines */}
          {layerLabels.map((l) => {
          ...
          {/* trajectories */}
          {r.tokens.map((_, t) => {
            const pts = r.trajectories
              .map(
                (layer) =>
                  `${px(layer[t][0]).toFixed(1)},${py(layer[t][1]).toFixed(1)}`,
              )
              .join(" ");
            return (
              <g key={t}>
                <polyline
                  points={pts}
                  fill="none"
                  stroke={TOKEN_COLORS[t % TOKEN_COLORS.length]}
                  strokeWidth="2"
                  opacity="0.85"
                ...
                {r.trajectories.map((layer, l) => (
                  <circle
                    ...
                    cx={px(layer[t][0])}
                    cy={py(layer[t][1])}
                    r={l === 0 || l === r.trajectories.length - 1 ? 4 : 2.4}
                    fill={TOKEN_COLORS[t % TOKEN_COLORS.length]}
                    stroke={
                      l === 0 || l === r.trajectories.length - 1
                        ? "#fff"
                        : "none"
                    }
                    strokeWidth="1"
                ...
                <text
                  x={px(r.trajectories[maxLayer][t][0]) + 8}
                  y={py(r.trajectories[maxLayer][t][1]) + 3}
                  fontSize="10"
                  fontWeight="600"
                  fill={TOKEN_COLORS[t % TOKEN_COLORS.length]}
                >
      ...
      <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-3xl p-4 md:p-6 shadow-ambient dark:shadow-dark-ambient border border-outline-variant/40 dark:border-white/10">
        ...
        <div className="flex items-end gap-1 h-40">
          {r.injections.map((inj) => {
            const maxI = Math.max(
              ...r.injections.map((x) => Math.max(x.attention, x.ffn)),
              0.01,
            );
            ...
            return (
              <div
                key={inj.layer}
                className="flex-1 flex flex-col justify-end items-center min-w-0 gap-0.5"
                title={`L${inj.layer}: attn ${inj.attention.toFixed(3)} ffn ${inj.ffn.toFixed(3)}`}
              >
                <div
        ...
        <div className="flex mt-1">
          {r.injections.map((inj) => (
            ...
            >
              {inj.layer % 3 === 0 ? inj.layer : ""}
            </span>
          ))}
        </div>
        <div className="mt-2 flex flex-wrap gap-4 text-caption text-on-surface-variant dark:text-outline">
          <span className="inline-flex items-center gap-1.5">
            <span className="w-3 h-2 rounded-sm bg-[#5B6BB0]" />{" "}
            {texts.ui.attention}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span className="w-3 h-2 rounded-sm bg-[#C8604A]" /> {texts.ui.ffn}
          </span>
          <span className="ml-auto font-mono">
            {fmt(texts.ui.verdictEvidence, {
              a: lastInj.attention.toFixed(2),
              f: lastInj.ffn.toFixed(2),
            })}{" "}
            @ L{lastInj.layer}
          </span>
        </div>
        ...
        {l2Goal && (
          <div>
            <div className="mb-2 text-caption text-on-surface-variant dark:text-outline">
              {texts.ui.l2CurInj}:{" "}
              <span className="font-mono text-primary dark:text-inverse-primary">
                attn={lastInj.attention.toFixed(3)}, ffn=
                {lastInj.ffn.toFixed(3)}
              </span>
            </div>
            <div
      ...
      <ABPanel
        ...
        b={shotB}
        onSnapA={() =>
          setShotA(makeShot(`A: attn=${lastInj.attention.toFixed(2)}`))
        }
        onSnapB={() =>
          setShotB(makeShot(`B: attn=${lastInj.attention.toFixed(2)}`))
        }
        onSwap={() => {
          setShotA(shotB);
          setShotB(shotA);
