...
function drawLoss(
  ...
  ctx.beginPath();
  ctx.arc(s.px(r.minimum.x), s.py(r.minimum.y), 6, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.fillText(
    `(${r.probe.x.toFixed(2)}, ${r.probe.y.toFixed(2)})`,
    px + 10,
    py - 8,
  );
...
export default function LossLab({
  ...
  const { handlers } = useCanvasDrag({
    ...
    onDown: (x) => {
      setParams({ ...params, probe: parseFloat(clampX(x).toFixed(2)) });
      return true;
    },
    onDrag: (x) => {
      setParams({ ...params, probe: parseFloat(clampX(x).toFixed(2)) });
