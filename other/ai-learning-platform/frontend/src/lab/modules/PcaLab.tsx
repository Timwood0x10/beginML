...
export default function PcaLab({ result }: { result: LabResult | null }) {
  ...
  useEffect(() => {
    ...
    for (const p of r.reconstructed) {
      ctx.beginPath();
      ctx.arc(s.px(p[0]), s.py(p[1]), 3, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    for (const p of r.points) {
      ctx.beginPath();
      ctx.arc(s.px(p[0]), s.py(p[1]), 3.5, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    ctx.beginPath();
    ctx.arc(s.px(mx), s.py(my), 5, 0, Math.PI * 2);
    ctx.fill();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-2xl p-5 border border-outline-variant/40 dark:border-white/10">
              ...
              <div key={ev.name}>
                ...
                <div className="h-2.5 bg-surface-variant dark:bg-white/10 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${ev.variance * 100}%`,
                      background: i === 0 ? "#7A5C36" : "#C8604A",
                    }}
                  />
                </div>
              </div>
          ...
          <div className="mt-4 pt-4 border-t border-outline-variant/50 dark:border-white/10 flex justify-between text-body-md">
            <span className="text-on-surface-variant dark:text-outline">
              Kept with {r.components} component{r.components === 1 ? "" : "s"}
            </span>
            <span className="font-headline font-bold text-primary dark:text-inverse-primary">
              {(r.keptVariance * 100).toFixed(1)}%
            </span>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest dark:bg-dark-surface rounded-2xl p-5 border border-outline-variant/40 dark:border-white/10">
          ...
          <p className="text-body-md text-on-surface-variant dark:text-outline mb-4 leading-relaxed">
            ...
            direction — and therefore how much information each component
            preserves.
          </p>
          <div className="flex gap-4">
            {r.eigenvalues.map((v, i) => (
              <div
                key={i}
                className="flex-1 bg-surface-container dark:bg-white/5 rounded-xl p-4 text-center"
              >
                <div className="font-mono text-2xl font-bold text-primary dark:text-inverse-primary">
                  {v.toFixed(2)}
                </div>
          ...
          <p className="text-caption text-outline mt-4">
            Slide <strong>Components kept</strong> to 0 to project onto the
            mean, 1 to drop the low-variance axis, or 2 to reconstruct
            perfectly.
          </p>
        </div>
      </div>
