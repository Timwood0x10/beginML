          ...
          <div className="inline-block min-w-full">
            <div className="flex ml-16">
              {r.tokens.map((t) => (
                <div
                  key={t}
                  className="w-11 md:w-13 text-center text-caption font-semibold text-on-surface-variant dark:text-outline pb-1"
                >
                  {t}
                </div>
              ))}
        ...
        {hover && !(r.causal && hover.col > hover.row) && (
          <p className="mt-3 text-caption text-on-surface-variant dark:text-outline">
            ...
            <span className="font-mono font-semibold">
              {matrix[hover.row][hover.col].toFixed(3)}
            </span>
          </p>
        )}
...
function LossCurve({
  ...
  const points = losses.map((v, i) => {
    const x =
      PAD.l + (i / Math.max(1, losses.length - 1)) * (W - PAD.l - PAD.r);
          ...
          return (
            <g key={t}>
              ...
              <text
                ...
                y={y + 4}
                textAnchor="end"
                className="fill-on-surface-variant dark:fill-outline"
                fontSize="11"
              >
                {val}
              </text>
