...
export default function LabPage() {
  ...
  useEffect(() => {
    const reload = () => {
      try {
        const raw = window.localStorage.getItem(QUEST_CHAIN_KEY);
        setQuestComplete(
          raw ? new Set<string>(JSON.parse(raw)) : new Set<string>(),
        );
      } catch {
  ...
  const onAction = (key: string) => {
    ...
    // sampler draws a fresh stochastic batch (Simulation Contract).
      ...
      setParams((p) => ({ ...p, seed: (Number(p.seed) || 1) + 1 }));
  ...
  if (modules.length === 0) {
    return (
      <div className="p-8 text-on-surface-variant dark:text-outline">
        {t.common.loading}
      </div>
    );
  }
  ...
  const unlocked = (id: string) =>
    modules.some((m) => m.next_experiment === id && completed(m.id));
  ...
  return (
    <div
      data-scope="tlab"
      className="flex flex-col md:flex-row gap-6 pt-2 -mx-margin-mobile md:mx-0"
    >
      <aside
        ...
        style={{
          background: "var(--tlab-panel)",
          borderColor: "var(--tlab-border)",
        }}
      >
        <div className="px-2 mb-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="font-headline text-headline-lg-mobile text-primary dark:text-inverse-primary leading-tight">
                {t.lab.title}
              </h2>
        ...
        <nav className="flex flex-col gap-4 mb-5">
          {orderedGroups.map((g) => (
            <div key={g}>
              <h4 className="px-2 mb-1 text-caption uppercase tracking-wider font-semibold text-outline">
                {labGroups[g] ?? g}
              </h4>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1 min-w-0">
                        <span className="block text-body-md truncate">
                          {labMeta[m.id]?.title ?? m.title}
                        </span>
                        <span
                          className={`block text-caption truncate ${isActive ? "opacity-80" : ""}`}
                        >
                          {labMeta[m.id]?.subtitle ?? m.subtitle}
                        </span>
                      </span>
                    </button>
                  );
              ...
              <div className="flex flex-col gap-4">
                {active.controls.map((c) => (
                  <ControlRow
                    key={c.key}
                    control={c}
                    label={controlLabels[c.key] ?? c.label}
                    value={params[c.key]}
      ...
      <main className="flex-1 min-w-0 flex flex-col gap-5">
        {active && (
          ...
          >
            ...
            <span className="text-caption uppercase tracking-wider font-semibold text-primary dark:text-inverse-primary">
              {t.home.categoryNames[active.category] ?? active.category}
            </span>
            <h1 className="font-headline text-headline-lg-mobile md:text-headline-xl text-on-surface dark:text-inverse-on-surface mt-1">
              {labMeta[active.id]?.title ?? active.title}
            </h1>
            <p className="text-body-md text-on-surface-variant dark:text-outline mt-2 max-w-3xl leading-relaxed">
              {labMeta[active.id]?.blurb ?? active.blurb}
            </p>
            ...
            {active.next_experiment &&
              (() => {
                const nxt = modules.find(
                  (m) => m.id === active.next_experiment,
                );
                return nxt ? (
                  <button
        ...
        {error ? (
          <div className="bg-error-container text-on-error-container rounded-2xl p-6 text-body-md">
            {error}
          </div>
        ) : Lab ? (
          <Lab
        ...
        <JournalPanel
          ...
          onExplore={
            active?.next_experiment
              ? () => navigate(`/lab/${active.next_experiment!}`)
              : undefined
          }
        />
      </main>
