...
function wrapOversizedElement(el: HTMLElement, maxHeight: number): HTMLElement {
  ...
  if (
    el.tagName === "TABLE" &&
    el.parentElement?.classList.contains("table-wrap")
  ) {
    el = el.parentElement;
  }
  // Avoid double-wrapping
...
export function useBookPagination(
  ...
  useEffect(() => {
    ...
    const paginate = () => {
      ...
      const contentHeight =
        pageHeight - padTop - padBottom - borderTop - borderBottom - 6;
      ...
      for (const child of children) {
        ...
        if (
          solo > contentHeight &&
          ["TABLE", "PRE", "UL", "OL", "IMG"].includes(
            el.tagName === "DIV" && el.classList.contains("table-wrap")
              ? "TABLE"
              : el.tagName === "DIV" && el.classList.contains("book-oversize")
                ? "DIV"
                : el.tagName,
          )
        ) {
